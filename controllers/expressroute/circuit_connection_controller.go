...
// CircuitConnectionReconciler reconciles a CircuitConnection object
type CircuitConnectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=expressroute.azurerm.kubeform.com,resources=circuitconnections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=expressroute.azurerm.kubeform.com,resources=circuitconnections/status,verbs=get;update;patch

func (r *CircuitConnectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("circuitconnection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CircuitConnection")
...
func (r *CircuitConnectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &expressroutev1alpha1.CircuitConnection{}); err != nil {
			klog.Error(err, "unable to set up auditor", expressroutev1alpha1.CircuitConnection{}.APIVersion, expressroutev1alpha1.CircuitConnection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&expressroutev1alpha1.CircuitConnection{}).
...
func (r *CircuitConnectionReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for CircuitConnection")
			return nil
		}
		...
		resourceList := &expressroutev1alpha1.CircuitConnectionList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list CircuitConnection")
			return nil
		}
