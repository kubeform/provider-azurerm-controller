...
// CircuitAuthorizationReconciler reconciles a CircuitAuthorization object
type CircuitAuthorizationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=expressroute.azurerm.kubeform.com,resources=circuitauthorizations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=expressroute.azurerm.kubeform.com,resources=circuitauthorizations/status,verbs=get;update;patch

func (r *CircuitAuthorizationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("circuitauthorization", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CircuitAuthorization")
...
func (r *CircuitAuthorizationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &expressroutev1alpha1.CircuitAuthorization{}); err != nil {
			klog.Error(err, "unable to set up auditor", expressroutev1alpha1.CircuitAuthorization{}.APIVersion, expressroutev1alpha1.CircuitAuthorization{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&expressroutev1alpha1.CircuitAuthorization{}).
...
func (r *CircuitAuthorizationReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for CircuitAuthorization")
			return nil
		}
		...
		resourceList := &expressroutev1alpha1.CircuitAuthorizationList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list CircuitAuthorization")
			return nil
		}
