...
// CircuitPeeringReconciler reconciles a CircuitPeering object
type CircuitPeeringReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=expressroute.azurerm.kubeform.com,resources=circuitpeerings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=expressroute.azurerm.kubeform.com,resources=circuitpeerings/status,verbs=get;update;patch

func (r *CircuitPeeringReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("circuitpeering", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CircuitPeering")
...
func (r *CircuitPeeringReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &expressroutev1alpha1.CircuitPeering{}); err != nil {
			klog.Error(err, "unable to set up auditor", expressroutev1alpha1.CircuitPeering{}.APIVersion, expressroutev1alpha1.CircuitPeering{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&expressroutev1alpha1.CircuitPeering{}).
...
func (r *CircuitPeeringReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for CircuitPeering")
			return nil
		}
		...
		resourceList := &expressroutev1alpha1.CircuitPeeringList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list CircuitPeering")
			return nil
		}
