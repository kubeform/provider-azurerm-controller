...
// ExpressRouteAuthorizationReconciler reconciles a ExpressRouteAuthorization object
type ExpressRouteAuthorizationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vmware.azurerm.kubeform.com,resources=expressrouteauthorizations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vmware.azurerm.kubeform.com,resources=expressrouteauthorizations/status,verbs=get;update;patch

func (r *ExpressRouteAuthorizationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("expressrouteauthorization", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ExpressRouteAuthorization")
...
func (r *ExpressRouteAuthorizationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vmwarev1alpha1.ExpressRouteAuthorization{}); err != nil {
			klog.Error(err, "unable to set up auditor", vmwarev1alpha1.ExpressRouteAuthorization{}.APIVersion, vmwarev1alpha1.ExpressRouteAuthorization{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vmwarev1alpha1.ExpressRouteAuthorization{}).
...
func (r *ExpressRouteAuthorizationReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ExpressRouteAuthorization")
			return nil
		}
		...
		resourceList := &vmwarev1alpha1.ExpressRouteAuthorizationList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ExpressRouteAuthorization")
			return nil
		}
