...
// PrivateCloudReconciler reconciles a PrivateCloud object
type PrivateCloudReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vmware.azurerm.kubeform.com,resources=privateclouds,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vmware.azurerm.kubeform.com,resources=privateclouds/status,verbs=get;update;patch

func (r *PrivateCloudReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("privatecloud", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PrivateCloud")
...
func (r *PrivateCloudReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vmwarev1alpha1.PrivateCloud{}); err != nil {
			klog.Error(err, "unable to set up auditor", vmwarev1alpha1.PrivateCloud{}.APIVersion, vmwarev1alpha1.PrivateCloud{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vmwarev1alpha1.PrivateCloud{}).
...
func (r *PrivateCloudReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for PrivateCloud")
			return nil
		}
		...
		resourceList := &vmwarev1alpha1.PrivateCloudList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list PrivateCloud")
			return nil
		}
