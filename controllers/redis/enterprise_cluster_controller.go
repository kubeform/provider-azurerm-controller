...
// EnterpriseClusterReconciler reconciles a EnterpriseCluster object
type EnterpriseClusterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=redis.azurerm.kubeform.com,resources=enterpriseclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=redis.azurerm.kubeform.com,resources=enterpriseclusters/status,verbs=get;update;patch

func (r *EnterpriseClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("enterprisecluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EnterpriseCluster")
...
func (r *EnterpriseClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &redisv1alpha1.EnterpriseCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", redisv1alpha1.EnterpriseCluster{}.APIVersion, redisv1alpha1.EnterpriseCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&redisv1alpha1.EnterpriseCluster{}).
