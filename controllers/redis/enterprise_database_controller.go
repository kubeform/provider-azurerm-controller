...
// EnterpriseDatabaseReconciler reconciles a EnterpriseDatabase object
type EnterpriseDatabaseReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=redis.azurerm.kubeform.com,resources=enterprisedatabases,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=redis.azurerm.kubeform.com,resources=enterprisedatabases/status,verbs=get;update;patch

func (r *EnterpriseDatabaseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("enterprisedatabase", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EnterpriseDatabase")
...
func (r *EnterpriseDatabaseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &redisv1alpha1.EnterpriseDatabase{}); err != nil {
			klog.Error(err, "unable to set up auditor", redisv1alpha1.EnterpriseDatabase{}.APIVersion, redisv1alpha1.EnterpriseDatabase{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&redisv1alpha1.EnterpriseDatabase{}).
...
func (r *EnterpriseDatabaseReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for EnterpriseDatabase")
			return nil
		}
		...
		resourceList := &redisv1alpha1.EnterpriseDatabaseList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list EnterpriseDatabase")
			return nil
		}
