...
// +kubebuilder:rbac:groups=redis.azurerm.kubeform.com,resources=caches,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=redis.azurerm.kubeform.com,resources=caches/status,verbs=get;update;patch
...
func (r *CacheReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &redisv1alpha1.Cache{}); err != nil {
			klog.Error(err, "unable to set up auditor", redisv1alpha1.Cache{}.APIVersion, redisv1alpha1.Cache{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&redisv1alpha1.Cache{}).
...
func (r *CacheReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Cache")
			return nil
		}
		...
		resourceList := &redisv1alpha1.CacheList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Cache")
			return nil
		}
