...
// LinkedServerReconciler reconciles a LinkedServer object
type LinkedServerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=redis.azurerm.kubeform.com,resources=linkedservers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=redis.azurerm.kubeform.com,resources=linkedservers/status,verbs=get;update;patch

func (r *LinkedServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("linkedserver", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LinkedServer")
...
func (r *LinkedServerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &redisv1alpha1.LinkedServer{}); err != nil {
			klog.Error(err, "unable to set up auditor", redisv1alpha1.LinkedServer{}.APIVersion, redisv1alpha1.LinkedServer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&redisv1alpha1.LinkedServer{}).
