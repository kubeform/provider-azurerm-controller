...
// WorkspaceCustomerManagedKeyReconciler reconciles a WorkspaceCustomerManagedKey object
type WorkspaceCustomerManagedKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=databricks.azurerm.kubeform.com,resources=workspacecustomermanagedkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=databricks.azurerm.kubeform.com,resources=workspacecustomermanagedkeys/status,verbs=get;update;patch

func (r *WorkspaceCustomerManagedKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("workspacecustomermanagedkey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WorkspaceCustomerManagedKey")
...
func (r *WorkspaceCustomerManagedKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databricksv1alpha1.WorkspaceCustomerManagedKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", databricksv1alpha1.WorkspaceCustomerManagedKey{}.APIVersion, databricksv1alpha1.WorkspaceCustomerManagedKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databricksv1alpha1.WorkspaceCustomerManagedKey{}).
