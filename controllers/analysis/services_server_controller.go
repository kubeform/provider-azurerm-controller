...
// ServicesServerReconciler reconciles a ServicesServer object
type ServicesServerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=analysis.azurerm.kubeform.com,resources=servicesservers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=analysis.azurerm.kubeform.com,resources=servicesservers/status,verbs=get;update;patch

func (r *ServicesServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicesserver", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServicesServer")
...
func (r *ServicesServerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &analysisv1alpha1.ServicesServer{}); err != nil {
			klog.Error(err, "unable to set up auditor", analysisv1alpha1.ServicesServer{}.APIVersion, analysisv1alpha1.ServicesServer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&analysisv1alpha1.ServicesServer{}).
...
func (r *ServicesServerReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ServicesServer")
			return nil
		}
		...
		resourceList := &analysisv1alpha1.ServicesServerList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ServicesServer")
			return nil
		}
