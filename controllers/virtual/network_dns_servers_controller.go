...
// NetworkDNSServersReconciler reconciles a NetworkDNSServers object
type NetworkDNSServersReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=virtual.azurerm.kubeform.com,resources=networkdnsservers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=virtual.azurerm.kubeform.com,resources=networkdnsservers/status,verbs=get;update;patch

func (r *NetworkDNSServersReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networkdnsservers", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkDNSServers")
...
func (r *NetworkDNSServersReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &virtualv1alpha1.NetworkDNSServers{}); err != nil {
			klog.Error(err, "unable to set up auditor", virtualv1alpha1.NetworkDNSServers{}.APIVersion, virtualv1alpha1.NetworkDNSServers{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&virtualv1alpha1.NetworkDNSServers{}).
