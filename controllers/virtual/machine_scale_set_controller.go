...
// MachineScaleSetReconciler reconciles a MachineScaleSet object
type MachineScaleSetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=virtual.azurerm.kubeform.com,resources=machinescalesets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=virtual.azurerm.kubeform.com,resources=machinescalesets/status,verbs=get;update;patch

func (r *MachineScaleSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("machinescaleset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MachineScaleSet")
...
func (r *MachineScaleSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &virtualv1alpha1.MachineScaleSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", virtualv1alpha1.MachineScaleSet{}.APIVersion, virtualv1alpha1.MachineScaleSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&virtualv1alpha1.MachineScaleSet{}).
...
func (r *MachineScaleSetReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for MachineScaleSet")
			return nil
		}
		...
		resourceList := &virtualv1alpha1.MachineScaleSetList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list MachineScaleSet")
			return nil
		}
