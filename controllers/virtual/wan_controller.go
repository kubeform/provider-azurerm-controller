...
// WanReconciler reconciles a Wan object
type WanReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=virtual.azurerm.kubeform.com,resources=wans,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=virtual.azurerm.kubeform.com,resources=wans/status,verbs=get;update;patch

func (r *WanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("wan", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Wan")
...
func (r *WanReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &virtualv1alpha1.Wan{}); err != nil {
			klog.Error(err, "unable to set up auditor", virtualv1alpha1.Wan{}.APIVersion, virtualv1alpha1.Wan{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&virtualv1alpha1.Wan{}).
