...
// MachineDataDiskAttachmentReconciler reconciles a MachineDataDiskAttachment object
type MachineDataDiskAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=virtual.azurerm.kubeform.com,resources=machinedatadiskattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=virtual.azurerm.kubeform.com,resources=machinedatadiskattachments/status,verbs=get;update;patch

func (r *MachineDataDiskAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("machinedatadiskattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MachineDataDiskAttachment")
...
func (r *MachineDataDiskAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &virtualv1alpha1.MachineDataDiskAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", virtualv1alpha1.MachineDataDiskAttachment{}.APIVersion, virtualv1alpha1.MachineDataDiskAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&virtualv1alpha1.MachineDataDiskAttachment{}).
