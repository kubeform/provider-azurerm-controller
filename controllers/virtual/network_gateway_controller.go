...
// NetworkGatewayReconciler reconciles a NetworkGateway object
...
// +kubebuilder:rbac:groups=virtual.azurerm.kubeform.com,resources=networkgateways,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=virtual.azurerm.kubeform.com,resources=networkgateways/status,verbs=get;update;patch

func (r *NetworkGatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networkgateway", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkGateway")
...
func (r *NetworkGatewayReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &virtualv1alpha1.NetworkGateway{}); err != nil {
			klog.Error(err, "unable to set up auditor", virtualv1alpha1.NetworkGateway{}.APIVersion, virtualv1alpha1.NetworkGateway{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&virtualv1alpha1.NetworkGateway{}).
