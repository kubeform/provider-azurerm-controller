...
// FactoryTriggerBlobEventReconciler reconciles a FactoryTriggerBlobEvent object
type FactoryTriggerBlobEventReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=factorytriggerblobevents,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=factorytriggerblobevents/status,verbs=get;update;patch

func (r *FactoryTriggerBlobEventReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("factorytriggerblobevent", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FactoryTriggerBlobEvent")
...
func (r *FactoryTriggerBlobEventReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.FactoryTriggerBlobEvent{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.FactoryTriggerBlobEvent{}.APIVersion, datav1alpha1.FactoryTriggerBlobEvent{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.FactoryTriggerBlobEvent{}).
