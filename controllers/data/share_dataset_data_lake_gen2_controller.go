...
// ShareDatasetDataLakeGen2Reconciler reconciles a ShareDatasetDataLakeGen2 object
type ShareDatasetDataLakeGen2Reconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=sharedatasetdatalakegen2s,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=sharedatasetdatalakegen2s/status,verbs=get;update;patch

func (r *ShareDatasetDataLakeGen2Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sharedatasetdatalakegen2", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ShareDatasetDataLakeGen2")
...
func (r *ShareDatasetDataLakeGen2Reconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.ShareDatasetDataLakeGen2{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.ShareDatasetDataLakeGen2{}.APIVersion, datav1alpha1.ShareDatasetDataLakeGen2{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.ShareDatasetDataLakeGen2{}).
