...
// FactoryLinkedServiceWebReconciler reconciles a FactoryLinkedServiceWeb object
type FactoryLinkedServiceWebReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=factorylinkedservicewebs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=factorylinkedservicewebs/status,verbs=get;update;patch

func (r *FactoryLinkedServiceWebReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("factorylinkedserviceweb", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FactoryLinkedServiceWeb")
...
func (r *FactoryLinkedServiceWebReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.FactoryLinkedServiceWeb{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.FactoryLinkedServiceWeb{}.APIVersion, datav1alpha1.FactoryLinkedServiceWeb{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.FactoryLinkedServiceWeb{}).
...
func (r *FactoryLinkedServiceWebReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for FactoryLinkedServiceWeb")
			return nil
		}
		...
		resourceList := &datav1alpha1.FactoryLinkedServiceWebList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list FactoryLinkedServiceWeb")
			return nil
		}
