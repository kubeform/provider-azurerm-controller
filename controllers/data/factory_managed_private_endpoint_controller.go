...
// FactoryManagedPrivateEndpointReconciler reconciles a FactoryManagedPrivateEndpoint object
type FactoryManagedPrivateEndpointReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=factorymanagedprivateendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=factorymanagedprivateendpoints/status,verbs=get;update;patch

func (r *FactoryManagedPrivateEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("factorymanagedprivateendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FactoryManagedPrivateEndpoint")
...
func (r *FactoryManagedPrivateEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.FactoryManagedPrivateEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.FactoryManagedPrivateEndpoint{}.APIVersion, datav1alpha1.FactoryManagedPrivateEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.FactoryManagedPrivateEndpoint{}).
