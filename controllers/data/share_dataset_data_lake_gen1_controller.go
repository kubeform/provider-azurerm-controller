...
// ShareDatasetDataLakeGen1Reconciler reconciles a ShareDatasetDataLakeGen1 object
type ShareDatasetDataLakeGen1Reconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=sharedatasetdatalakegen1s,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=sharedatasetdatalakegen1s/status,verbs=get;update;patch

func (r *ShareDatasetDataLakeGen1Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sharedatasetdatalakegen1", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ShareDatasetDataLakeGen1")
...
func (r *ShareDatasetDataLakeGen1Reconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.ShareDatasetDataLakeGen1{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.ShareDatasetDataLakeGen1{}.APIVersion, datav1alpha1.ShareDatasetDataLakeGen1{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.ShareDatasetDataLakeGen1{}).
