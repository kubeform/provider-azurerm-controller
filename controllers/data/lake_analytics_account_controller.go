...
// LakeAnalyticsAccountReconciler reconciles a LakeAnalyticsAccount object
type LakeAnalyticsAccountReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=lakeanalyticsaccounts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=lakeanalyticsaccounts/status,verbs=get;update;patch

func (r *LakeAnalyticsAccountReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("lakeanalyticsaccount", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LakeAnalyticsAccount")
...
func (r *LakeAnalyticsAccountReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.LakeAnalyticsAccount{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.LakeAnalyticsAccount{}.APIVersion, datav1alpha1.LakeAnalyticsAccount{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.LakeAnalyticsAccount{}).
