...
// FactoryTriggerCustomEventReconciler reconciles a FactoryTriggerCustomEvent object
type FactoryTriggerCustomEventReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=factorytriggercustomevents,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=factorytriggercustomevents/status,verbs=get;update;patch

func (r *FactoryTriggerCustomEventReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("factorytriggercustomevent", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FactoryTriggerCustomEvent")
...
func (r *FactoryTriggerCustomEventReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.FactoryTriggerCustomEvent{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.FactoryTriggerCustomEvent{}.APIVersion, datav1alpha1.FactoryTriggerCustomEvent{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.FactoryTriggerCustomEvent{}).
