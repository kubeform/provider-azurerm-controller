...
// FactoryDatasetParquetReconciler reconciles a FactoryDatasetParquet object
type FactoryDatasetParquetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=factorydatasetparquets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.azurerm.kubeform.com,resources=factorydatasetparquets/status,verbs=get;update;patch

func (r *FactoryDatasetParquetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("factorydatasetparquet", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FactoryDatasetParquet")
...
func (r *FactoryDatasetParquetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.FactoryDatasetParquet{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.FactoryDatasetParquet{}.APIVersion, datav1alpha1.FactoryDatasetParquet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.FactoryDatasetParquet{}).
