...
// FlexibleDatabaseReconciler reconciles a FlexibleDatabase object
type FlexibleDatabaseReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=mysql.azurerm.kubeform.com,resources=flexibledatabases,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mysql.azurerm.kubeform.com,resources=flexibledatabases/status,verbs=get;update;patch

func (r *FlexibleDatabaseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("flexibledatabase", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FlexibleDatabase")
...
func (r *FlexibleDatabaseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mysqlv1alpha1.FlexibleDatabase{}); err != nil {
			klog.Error(err, "unable to set up auditor", mysqlv1alpha1.FlexibleDatabase{}.APIVersion, mysqlv1alpha1.FlexibleDatabase{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mysqlv1alpha1.FlexibleDatabase{}).
