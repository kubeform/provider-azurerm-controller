...
// NetworkSecurityGroupAssociationReconciler reconciles a NetworkSecurityGroupAssociation object
type NetworkSecurityGroupAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=subnet.azurerm.kubeform.com,resources=networksecuritygroupassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=subnet.azurerm.kubeform.com,resources=networksecuritygroupassociations/status,verbs=get;update;patch

func (r *NetworkSecurityGroupAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networksecuritygroupassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkSecurityGroupAssociation")
...
func (r *NetworkSecurityGroupAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &subnetv1alpha1.NetworkSecurityGroupAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", subnetv1alpha1.NetworkSecurityGroupAssociation{}.APIVersion, subnetv1alpha1.NetworkSecurityGroupAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&subnetv1alpha1.NetworkSecurityGroupAssociation{}).
