...
// NatGatewayAssociationReconciler reconciles a NatGatewayAssociation object
type NatGatewayAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=subnet.azurerm.kubeform.com,resources=natgatewayassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=subnet.azurerm.kubeform.com,resources=natgatewayassociations/status,verbs=get;update;patch

func (r *NatGatewayAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("natgatewayassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NatGatewayAssociation")
...
func (r *NatGatewayAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &subnetv1alpha1.NatGatewayAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", subnetv1alpha1.NatGatewayAssociation{}.APIVersion, subnetv1alpha1.NatGatewayAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&subnetv1alpha1.NatGatewayAssociation{}).
