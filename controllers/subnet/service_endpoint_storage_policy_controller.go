...
// ServiceEndpointStoragePolicyReconciler reconciles a ServiceEndpointStoragePolicy object
type ServiceEndpointStoragePolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=subnet.azurerm.kubeform.com,resources=serviceendpointstoragepolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=subnet.azurerm.kubeform.com,resources=serviceendpointstoragepolicies/status,verbs=get;update;patch

func (r *ServiceEndpointStoragePolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serviceendpointstoragepolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceEndpointStoragePolicy")
...
func (r *ServiceEndpointStoragePolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &subnetv1alpha1.ServiceEndpointStoragePolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", subnetv1alpha1.ServiceEndpointStoragePolicy{}.APIVersion, subnetv1alpha1.ServiceEndpointStoragePolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&subnetv1alpha1.ServiceEndpointStoragePolicy{}).
