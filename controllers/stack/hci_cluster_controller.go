...
// HciClusterReconciler reconciles a HciCluster object
type HciClusterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=stack.azurerm.kubeform.com,resources=hciclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=stack.azurerm.kubeform.com,resources=hciclusters/status,verbs=get;update;patch

func (r *HciClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hcicluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HciCluster")
...
func (r *HciClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &stackv1alpha1.HciCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", stackv1alpha1.HciCluster{}.APIVersion, stackv1alpha1.HciCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&stackv1alpha1.HciCluster{}).
