...
// ServicePublicCertificateReconciler reconciles a ServicePublicCertificate object
type ServicePublicCertificateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=servicepubliccertificates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=servicepubliccertificates/status,verbs=get;update;patch

func (r *ServicePublicCertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicepubliccertificate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServicePublicCertificate")
...
func (r *ServicePublicCertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appv1alpha1.ServicePublicCertificate{}); err != nil {
			klog.Error(err, "unable to set up auditor", appv1alpha1.ServicePublicCertificate{}.APIVersion, appv1alpha1.ServicePublicCertificate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appv1alpha1.ServicePublicCertificate{}).
