...
// ServiceHybridConnectionReconciler reconciles a ServiceHybridConnection object
type ServiceHybridConnectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=servicehybridconnections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=servicehybridconnections/status,verbs=get;update;patch

func (r *ServiceHybridConnectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicehybridconnection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceHybridConnection")
...
func (r *ServiceHybridConnectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appv1alpha1.ServiceHybridConnection{}); err != nil {
			klog.Error(err, "unable to set up auditor", appv1alpha1.ServiceHybridConnection{}.APIVersion, appv1alpha1.ServiceHybridConnection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appv1alpha1.ServiceHybridConnection{}).
...
func (r *ServiceHybridConnectionReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ServiceHybridConnection")
			return nil
		}
		...
		resourceList := &appv1alpha1.ServiceHybridConnectionList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ServiceHybridConnection")
			return nil
		}
