...
// ServiceEnvironmentV3Reconciler reconciles a ServiceEnvironmentV3 object
type ServiceEnvironmentV3Reconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=serviceenvironmentv3s,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=serviceenvironmentv3s/status,verbs=get;update;patch

func (r *ServiceEnvironmentV3Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serviceenvironmentv3", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceEnvironmentV3")
...
func (r *ServiceEnvironmentV3Reconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appv1alpha1.ServiceEnvironmentV3{}); err != nil {
			klog.Error(err, "unable to set up auditor", appv1alpha1.ServiceEnvironmentV3{}.APIVersion, appv1alpha1.ServiceEnvironmentV3{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appv1alpha1.ServiceEnvironmentV3{}).
