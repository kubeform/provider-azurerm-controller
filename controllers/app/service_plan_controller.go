...
// ServicePlanReconciler reconciles a ServicePlan object
...
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=serviceplans,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=serviceplans/status,verbs=get;update;patch

func (r *ServicePlanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serviceplan", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServicePlan")
...
func (r *ServicePlanReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appv1alpha1.ServicePlan{}); err != nil {
			klog.Error(err, "unable to set up auditor", appv1alpha1.ServicePlan{}.APIVersion, appv1alpha1.ServicePlan{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appv1alpha1.ServicePlan{}).
