...
// ServiceManagedCertificateReconciler reconciles a ServiceManagedCertificate object
type ServiceManagedCertificateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=servicemanagedcertificates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=servicemanagedcertificates/status,verbs=get;update;patch

func (r *ServiceManagedCertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicemanagedcertificate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceManagedCertificate")
...
func (r *ServiceManagedCertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appv1alpha1.ServiceManagedCertificate{}); err != nil {
			klog.Error(err, "unable to set up auditor", appv1alpha1.ServiceManagedCertificate{}.APIVersion, appv1alpha1.ServiceManagedCertificate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appv1alpha1.ServiceManagedCertificate{}).
