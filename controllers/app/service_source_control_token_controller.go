...
// ServiceSourceControlTokenReconciler reconciles a ServiceSourceControlToken object
type ServiceSourceControlTokenReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=servicesourcecontroltokens,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=servicesourcecontroltokens/status,verbs=get;update;patch

func (r *ServiceSourceControlTokenReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicesourcecontroltoken", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceSourceControlToken")
...
func (r *ServiceSourceControlTokenReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appv1alpha1.ServiceSourceControlToken{}); err != nil {
			klog.Error(err, "unable to set up auditor", appv1alpha1.ServiceSourceControlToken{}.APIVersion, appv1alpha1.ServiceSourceControlToken{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appv1alpha1.ServiceSourceControlToken{}).
...
func (r *ServiceSourceControlTokenReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ServiceSourceControlToken")
			return nil
		}
		...
		resourceList := &appv1alpha1.ServiceSourceControlTokenList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ServiceSourceControlToken")
			return nil
		}
