...
// ServiceActiveSlotReconciler reconciles a ServiceActiveSlot object
type ServiceActiveSlotReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=serviceactiveslots,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=app.azurerm.kubeform.com,resources=serviceactiveslots/status,verbs=get;update;patch

func (r *ServiceActiveSlotReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serviceactiveslot", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceActiveSlot")
...
func (r *ServiceActiveSlotReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appv1alpha1.ServiceActiveSlot{}); err != nil {
			klog.Error(err, "unable to set up auditor", appv1alpha1.ServiceActiveSlot{}.APIVersion, appv1alpha1.ServiceActiveSlot{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appv1alpha1.ServiceActiveSlot{}).
