...
// PoolIscsiTargetReconciler reconciles a PoolIscsiTarget object
type PoolIscsiTargetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=disk.azurerm.kubeform.com,resources=pooliscsitargets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=disk.azurerm.kubeform.com,resources=pooliscsitargets/status,verbs=get;update;patch

func (r *PoolIscsiTargetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pooliscsitarget", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PoolIscsiTarget")
...
func (r *PoolIscsiTargetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &diskv1alpha1.PoolIscsiTarget{}); err != nil {
			klog.Error(err, "unable to set up auditor", diskv1alpha1.PoolIscsiTarget{}.APIVersion, diskv1alpha1.PoolIscsiTarget{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&diskv1alpha1.PoolIscsiTarget{}).
