...
// PoolManagedDiskAttachmentReconciler reconciles a PoolManagedDiskAttachment object
type PoolManagedDiskAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=disk.azurerm.kubeform.com,resources=poolmanageddiskattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=disk.azurerm.kubeform.com,resources=poolmanageddiskattachments/status,verbs=get;update;patch

func (r *PoolManagedDiskAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("poolmanageddiskattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PoolManagedDiskAttachment")
...
func (r *PoolManagedDiskAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &diskv1alpha1.PoolManagedDiskAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", diskv1alpha1.PoolManagedDiskAttachment{}.APIVersion, diskv1alpha1.PoolManagedDiskAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&diskv1alpha1.PoolManagedDiskAttachment{}).
