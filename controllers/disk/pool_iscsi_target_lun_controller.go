...
// PoolIscsiTargetLunReconciler reconciles a PoolIscsiTargetLun object
type PoolIscsiTargetLunReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=disk.azurerm.kubeform.com,resources=pooliscsitargetluns,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=disk.azurerm.kubeform.com,resources=pooliscsitargetluns/status,verbs=get;update;patch

func (r *PoolIscsiTargetLunReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pooliscsitargetlun", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PoolIscsiTargetLun")
...
func (r *PoolIscsiTargetLunReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &diskv1alpha1.PoolIscsiTargetLun{}); err != nil {
			klog.Error(err, "unable to set up auditor", diskv1alpha1.PoolIscsiTargetLun{}.APIVersion, diskv1alpha1.PoolIscsiTargetLun{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&diskv1alpha1.PoolIscsiTargetLun{}).
