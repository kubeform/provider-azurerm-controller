...
// GlobalVmShutdownScheduleReconciler reconciles a GlobalVmShutdownSchedule object
type GlobalVmShutdownScheduleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=devtest.azurerm.kubeform.com,resources=globalvmshutdownschedules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=devtest.azurerm.kubeform.com,resources=globalvmshutdownschedules/status,verbs=get;update;patch

func (r *GlobalVmShutdownScheduleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("globalvmshutdownschedule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GlobalVmShutdownSchedule")
...
func (r *GlobalVmShutdownScheduleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devtestv1alpha1.GlobalVmShutdownSchedule{}); err != nil {
			klog.Error(err, "unable to set up auditor", devtestv1alpha1.GlobalVmShutdownSchedule{}.APIVersion, devtestv1alpha1.GlobalVmShutdownSchedule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devtestv1alpha1.GlobalVmShutdownSchedule{}).
