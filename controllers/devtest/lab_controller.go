...
// +kubebuilder:rbac:groups=devtest.azurerm.kubeform.com,resources=labs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=devtest.azurerm.kubeform.com,resources=labs/status,verbs=get;update;patch

func (r *LabReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("lab", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Lab")
...
func (r *LabReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devtestv1alpha1.Lab{}); err != nil {
			klog.Error(err, "unable to set up auditor", devtestv1alpha1.Lab{}.APIVersion, devtestv1alpha1.Lab{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devtestv1alpha1.Lab{}).
