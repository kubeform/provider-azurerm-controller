...
// WindowsVirtualMachineReconciler reconciles a WindowsVirtualMachine object
type WindowsVirtualMachineReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=devtest.azurerm.kubeform.com,resources=windowsvirtualmachines,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=devtest.azurerm.kubeform.com,resources=windowsvirtualmachines/status,verbs=get;update;patch

func (r *WindowsVirtualMachineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("windowsvirtualmachine", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WindowsVirtualMachine")
...
func (r *WindowsVirtualMachineReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devtestv1alpha1.WindowsVirtualMachine{}); err != nil {
			klog.Error(err, "unable to set up auditor", devtestv1alpha1.WindowsVirtualMachine{}.APIVersion, devtestv1alpha1.WindowsVirtualMachine{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devtestv1alpha1.WindowsVirtualMachine{}).
