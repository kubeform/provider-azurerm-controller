...
// AssignedIdentityReconciler reconciles a AssignedIdentity object
type AssignedIdentityReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=user.azurerm.kubeform.com,resources=assignedidentities,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=user.azurerm.kubeform.com,resources=assignedidentities/status,verbs=get;update;patch

func (r *AssignedIdentityReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("assignedidentity", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AssignedIdentity")
...
func (r *AssignedIdentityReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &userv1alpha1.AssignedIdentity{}); err != nil {
			klog.Error(err, "unable to set up auditor", userv1alpha1.AssignedIdentity{}.APIVersion, userv1alpha1.AssignedIdentity{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&userv1alpha1.AssignedIdentity{}).
