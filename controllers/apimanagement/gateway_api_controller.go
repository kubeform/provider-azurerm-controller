...
// +kubebuilder:rbac:groups=apimanagement.azurerm.kubeform.com,resources=gatewayapis,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apimanagement.azurerm.kubeform.com,resources=gatewayapis/status,verbs=get;update;patch

func (r *GatewayAPIReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gatewayapi", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GatewayAPI")
...
func (r *GatewayAPIReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apimanagementv1alpha1.GatewayAPI{}); err != nil {
			klog.Error(err, "unable to set up auditor", apimanagementv1alpha1.GatewayAPI{}.APIVersion, apimanagementv1alpha1.GatewayAPI{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apimanagementv1alpha1.GatewayAPI{}).
