...
// IdentityProviderAadReconciler reconciles a IdentityProviderAad object
type IdentityProviderAadReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apimanagement.azurerm.kubeform.com,resources=identityprovideraads,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apimanagement.azurerm.kubeform.com,resources=identityprovideraads/status,verbs=get;update;patch

func (r *IdentityProviderAadReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("identityprovideraad", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IdentityProviderAad")
...
func (r *IdentityProviderAadReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apimanagementv1alpha1.IdentityProviderAad{}); err != nil {
			klog.Error(err, "unable to set up auditor", apimanagementv1alpha1.IdentityProviderAad{}.APIVersion, apimanagementv1alpha1.IdentityProviderAad{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apimanagementv1alpha1.IdentityProviderAad{}).
...
func (r *IdentityProviderAadReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for IdentityProviderAad")
			return nil
		}
		...
		resourceList := &apimanagementv1alpha1.IdentityProviderAadList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list IdentityProviderAad")
			return nil
		}
