...
// EmailTemplateReconciler reconciles a EmailTemplate object
type EmailTemplateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apimanagement.azurerm.kubeform.com,resources=emailtemplates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apimanagement.azurerm.kubeform.com,resources=emailtemplates/status,verbs=get;update;patch

func (r *EmailTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("emailtemplate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EmailTemplate")
...
func (r *EmailTemplateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apimanagementv1alpha1.EmailTemplate{}); err != nil {
			klog.Error(err, "unable to set up auditor", apimanagementv1alpha1.EmailTemplate{}.APIVersion, apimanagementv1alpha1.EmailTemplate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apimanagementv1alpha1.EmailTemplate{}).
