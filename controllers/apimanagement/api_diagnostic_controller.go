...
// ApiDiagnosticReconciler reconciles a ApiDiagnostic object
type ApiDiagnosticReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apimanagement.azurerm.kubeform.com,resources=apidiagnostics,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apimanagement.azurerm.kubeform.com,resources=apidiagnostics/status,verbs=get;update;patch

func (r *ApiDiagnosticReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("apidiagnostic", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApiDiagnostic")
...
func (r *ApiDiagnosticReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apimanagementv1alpha1.ApiDiagnostic{}); err != nil {
			klog.Error(err, "unable to set up auditor", apimanagementv1alpha1.ApiDiagnostic{}.APIVersion, apimanagementv1alpha1.ApiDiagnostic{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apimanagementv1alpha1.ApiDiagnostic{}).
