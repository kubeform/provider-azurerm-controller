...
// IdentityProviderFacebookReconciler reconciles a IdentityProviderFacebook object
type IdentityProviderFacebookReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apimanagement.azurerm.kubeform.com,resources=identityproviderfacebooks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apimanagement.azurerm.kubeform.com,resources=identityproviderfacebooks/status,verbs=get;update;patch

func (r *IdentityProviderFacebookReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("identityproviderfacebook", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IdentityProviderFacebook")
...
func (r *IdentityProviderFacebookReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apimanagementv1alpha1.IdentityProviderFacebook{}); err != nil {
			klog.Error(err, "unable to set up auditor", apimanagementv1alpha1.IdentityProviderFacebook{}.APIVersion, apimanagementv1alpha1.IdentityProviderFacebook{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apimanagementv1alpha1.IdentityProviderFacebook{}).
...
func (r *IdentityProviderFacebookReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for IdentityProviderFacebook")
			return nil
		}
		...
		resourceList := &apimanagementv1alpha1.IdentityProviderFacebookList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list IdentityProviderFacebook")
			return nil
		}
