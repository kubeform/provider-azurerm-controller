...
// ProductPolicyReconciler reconciles a ProductPolicy object
type ProductPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apimanagement.azurerm.kubeform.com,resources=productpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apimanagement.azurerm.kubeform.com,resources=productpolicies/status,verbs=get;update;patch

func (r *ProductPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("productpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProductPolicy")
...
func (r *ProductPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apimanagementv1alpha1.ProductPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", apimanagementv1alpha1.ProductPolicy{}.APIVersion, apimanagementv1alpha1.ProductPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apimanagementv1alpha1.ProductPolicy{}).
