...
// BlobReconciler reconciles a Blob object
...
// +kubebuilder:rbac:groups=storage.azurerm.kubeform.com,resources=blobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storage.azurerm.kubeform.com,resources=blobs/status,verbs=get;update;patch

func (r *BlobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("blob", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Blob")
...
func (r *BlobReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagev1alpha1.Blob{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagev1alpha1.Blob{}.APIVersion, storagev1alpha1.Blob{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagev1alpha1.Blob{}).
