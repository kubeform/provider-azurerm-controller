...
// DataLakeGen2PathReconciler reconciles a DataLakeGen2Path object
type DataLakeGen2PathReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storage.azurerm.kubeform.com,resources=datalakegen2paths,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storage.azurerm.kubeform.com,resources=datalakegen2paths/status,verbs=get;update;patch

func (r *DataLakeGen2PathReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("datalakegen2path", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DataLakeGen2Path")
...
func (r *DataLakeGen2PathReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagev1alpha1.DataLakeGen2Path{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagev1alpha1.DataLakeGen2Path{}.APIVersion, storagev1alpha1.DataLakeGen2Path{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagev1alpha1.DataLakeGen2Path{}).
