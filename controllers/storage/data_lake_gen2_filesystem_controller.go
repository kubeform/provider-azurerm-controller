...
// DataLakeGen2FilesystemReconciler reconciles a DataLakeGen2Filesystem object
type DataLakeGen2FilesystemReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storage.azurerm.kubeform.com,resources=datalakegen2filesystems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storage.azurerm.kubeform.com,resources=datalakegen2filesystems/status,verbs=get;update;patch

func (r *DataLakeGen2FilesystemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("datalakegen2filesystem", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DataLakeGen2Filesystem")
...
func (r *DataLakeGen2FilesystemReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagev1alpha1.DataLakeGen2Filesystem{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagev1alpha1.DataLakeGen2Filesystem{}.APIVersion, storagev1alpha1.DataLakeGen2Filesystem{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagev1alpha1.DataLakeGen2Filesystem{}).
