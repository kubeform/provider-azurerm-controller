...
// AccountNetworkRulesReconciler reconciles a AccountNetworkRules object
type AccountNetworkRulesReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storage.azurerm.kubeform.com,resources=accountnetworkrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storage.azurerm.kubeform.com,resources=accountnetworkrules/status,verbs=get;update;patch

func (r *AccountNetworkRulesReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accountnetworkrules", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccountNetworkRules")
...
func (r *AccountNetworkRulesReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagev1alpha1.AccountNetworkRules{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagev1alpha1.AccountNetworkRules{}.APIVersion, storagev1alpha1.AccountNetworkRules{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagev1alpha1.AccountNetworkRules{}).
