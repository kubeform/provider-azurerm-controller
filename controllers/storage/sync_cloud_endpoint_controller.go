...
// SyncCloudEndpointReconciler reconciles a SyncCloudEndpoint object
type SyncCloudEndpointReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storage.azurerm.kubeform.com,resources=synccloudendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storage.azurerm.kubeform.com,resources=synccloudendpoints/status,verbs=get;update;patch

func (r *SyncCloudEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("synccloudendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SyncCloudEndpoint")
...
func (r *SyncCloudEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagev1alpha1.SyncCloudEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagev1alpha1.SyncCloudEndpoint{}.APIVersion, storagev1alpha1.SyncCloudEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagev1alpha1.SyncCloudEndpoint{}).
