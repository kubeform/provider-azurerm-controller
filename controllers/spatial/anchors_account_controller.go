...
// AnchorsAccountReconciler reconciles a AnchorsAccount object
type AnchorsAccountReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=spatial.azurerm.kubeform.com,resources=anchorsaccounts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=spatial.azurerm.kubeform.com,resources=anchorsaccounts/status,verbs=get;update;patch

func (r *AnchorsAccountReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("anchorsaccount", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnchorsAccount")
...
func (r *AnchorsAccountReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &spatialv1alpha1.AnchorsAccount{}); err != nil {
			klog.Error(err, "unable to set up auditor", spatialv1alpha1.AnchorsAccount{}.APIVersion, spatialv1alpha1.AnchorsAccount{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&spatialv1alpha1.AnchorsAccount{}).
