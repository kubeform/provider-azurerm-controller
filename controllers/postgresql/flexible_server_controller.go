...
// FlexibleServerReconciler reconciles a FlexibleServer object
type FlexibleServerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=postgresql.azurerm.kubeform.com,resources=flexibleservers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=postgresql.azurerm.kubeform.com,resources=flexibleservers/status,verbs=get;update;patch

func (r *FlexibleServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("flexibleserver", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FlexibleServer")
...
func (r *FlexibleServerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &postgresqlv1alpha1.FlexibleServer{}); err != nil {
			klog.Error(err, "unable to set up auditor", postgresqlv1alpha1.FlexibleServer{}.APIVersion, postgresqlv1alpha1.FlexibleServer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&postgresqlv1alpha1.FlexibleServer{}).
...
func (r *FlexibleServerReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for FlexibleServer")
			return nil
		}
		...
		resourceList := &postgresqlv1alpha1.FlexibleServerList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list FlexibleServer")
			return nil
		}
