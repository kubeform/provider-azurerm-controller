...
// VirtualNetworkRuleReconciler reconciles a VirtualNetworkRule object
type VirtualNetworkRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=postgresql.azurerm.kubeform.com,resources=virtualnetworkrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=postgresql.azurerm.kubeform.com,resources=virtualnetworkrules/status,verbs=get;update;patch

func (r *VirtualNetworkRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("virtualnetworkrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VirtualNetworkRule")
...
func (r *VirtualNetworkRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &postgresqlv1alpha1.VirtualNetworkRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", postgresqlv1alpha1.VirtualNetworkRule{}.APIVersion, postgresqlv1alpha1.VirtualNetworkRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&postgresqlv1alpha1.VirtualNetworkRule{}).
