...
// ServerKeyReconciler reconciles a ServerKey object
type ServerKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=postgresql.azurerm.kubeform.com,resources=serverkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=postgresql.azurerm.kubeform.com,resources=serverkeys/status,verbs=get;update;patch

func (r *ServerKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serverkey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServerKey")
...
func (r *ServerKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &postgresqlv1alpha1.ServerKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", postgresqlv1alpha1.ServerKey{}.APIVersion, postgresqlv1alpha1.ServerKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&postgresqlv1alpha1.ServerKey{}).
