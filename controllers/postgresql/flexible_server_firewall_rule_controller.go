...
// FlexibleServerFirewallRuleReconciler reconciles a FlexibleServerFirewallRule object
type FlexibleServerFirewallRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=postgresql.azurerm.kubeform.com,resources=flexibleserverfirewallrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=postgresql.azurerm.kubeform.com,resources=flexibleserverfirewallrules/status,verbs=get;update;patch

func (r *FlexibleServerFirewallRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("flexibleserverfirewallrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FlexibleServerFirewallRule")
...
func (r *FlexibleServerFirewallRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &postgresqlv1alpha1.FlexibleServerFirewallRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", postgresqlv1alpha1.FlexibleServerFirewallRule{}.APIVersion, postgresqlv1alpha1.FlexibleServerFirewallRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&postgresqlv1alpha1.FlexibleServerFirewallRule{}).
