...
// DpsReconciler reconciles a Dps object
type DpsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iothub.azurerm.kubeform.com,resources=dps,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iothub.azurerm.kubeform.com,resources=dps/status,verbs=get;update;patch

func (r *DpsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dps", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Dps")
...
func (r *DpsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iothubv1alpha1.Dps{}); err != nil {
			klog.Error(err, "unable to set up auditor", iothubv1alpha1.Dps{}.APIVersion, iothubv1alpha1.Dps{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iothubv1alpha1.Dps{}).
...
func (r *DpsReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Dps")
			return nil
		}
