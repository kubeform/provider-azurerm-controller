...
type EndpointServicebusQueueReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iothub.azurerm.kubeform.com,resources=endpointservicebusqueues/status,verbs=get;update;patch
...
func (r *EndpointServicebusQueueReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EndpointServicebusQueue")
...
func (r *EndpointServicebusQueueReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&iothubv1alpha1.EndpointServicebusQueue{}).
...
func (r *EndpointServicebusQueueReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for EndpointServicebusQueue")
			return nil
		}
