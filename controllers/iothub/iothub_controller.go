...
// IothubReconciler reconciles a Iothub object
type IothubReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iothub.azurerm.kubeform.com,resources=iothubs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iothub.azurerm.kubeform.com,resources=iothubs/status,verbs=get;update;patch

func (r *IothubReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("iothub", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Iothub")
...
func (r *IothubReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iothubv1alpha1.Iothub{}); err != nil {
			klog.Error(err, "unable to set up auditor", iothubv1alpha1.Iothub{}.APIVersion, iothubv1alpha1.Iothub{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iothubv1alpha1.Iothub{}).
...
func (r *IothubReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Iothub")
			return nil
		}
		...
		resourceList := &iothubv1alpha1.IothubList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Iothub")
			return nil
		}
