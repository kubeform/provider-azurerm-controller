...
// SetDefinitionReconciler reconciles a SetDefinition object
type SetDefinitionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=policy.azurerm.kubeform.com,resources=setdefinitions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=policy.azurerm.kubeform.com,resources=setdefinitions/status,verbs=get;update;patch

func (r *SetDefinitionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("setdefinition", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SetDefinition")
...
func (r *SetDefinitionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &policyv1alpha1.SetDefinition{}); err != nil {
			klog.Error(err, "unable to set up auditor", policyv1alpha1.SetDefinition{}.APIVersion, policyv1alpha1.SetDefinition{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&policyv1alpha1.SetDefinition{}).
