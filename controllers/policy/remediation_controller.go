...
// +kubebuilder:rbac:groups=policy.azurerm.kubeform.com,resources=remediations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=policy.azurerm.kubeform.com,resources=remediations/status,verbs=get;update;patch
...
func (r *RemediationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Remediation")
...
func (r *RemediationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &policyv1alpha1.Remediation{}); err != nil {
			klog.Error(err, "unable to set up auditor", policyv1alpha1.Remediation{}.APIVersion, policyv1alpha1.Remediation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&policyv1alpha1.Remediation{}).
