...
// AnalyticsOutputTableReconciler reconciles a AnalyticsOutputTable object
type AnalyticsOutputTableReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsoutputtables,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsoutputtables/status,verbs=get;update;patch

func (r *AnalyticsOutputTableReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyticsoutputtable", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyticsOutputTable")
...
func (r *AnalyticsOutputTableReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &streamv1alpha1.AnalyticsOutputTable{}); err != nil {
			klog.Error(err, "unable to set up auditor", streamv1alpha1.AnalyticsOutputTable{}.APIVersion, streamv1alpha1.AnalyticsOutputTable{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&streamv1alpha1.AnalyticsOutputTable{}).
...
func (r *AnalyticsOutputTableReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for AnalyticsOutputTable")
			return nil
		}
		...
		resourceList := &streamv1alpha1.AnalyticsOutputTableList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AnalyticsOutputTable")
			return nil
		}
