...
// AnalyticsOutputBlobReconciler reconciles a AnalyticsOutputBlob object
type AnalyticsOutputBlobReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsoutputblobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsoutputblobs/status,verbs=get;update;patch

func (r *AnalyticsOutputBlobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyticsoutputblob", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyticsOutputBlob")
...
func (r *AnalyticsOutputBlobReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &streamv1alpha1.AnalyticsOutputBlob{}); err != nil {
			klog.Error(err, "unable to set up auditor", streamv1alpha1.AnalyticsOutputBlob{}.APIVersion, streamv1alpha1.AnalyticsOutputBlob{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&streamv1alpha1.AnalyticsOutputBlob{}).
...
func (r *AnalyticsOutputBlobReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for AnalyticsOutputBlob")
			return nil
		}
		...
		resourceList := &streamv1alpha1.AnalyticsOutputBlobList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AnalyticsOutputBlob")
			return nil
		}
