...
// AnalyticsStreamInputBlobReconciler reconciles a AnalyticsStreamInputBlob object
type AnalyticsStreamInputBlobReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsstreaminputblobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsstreaminputblobs/status,verbs=get;update;patch

func (r *AnalyticsStreamInputBlobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyticsstreaminputblob", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyticsStreamInputBlob")
...
func (r *AnalyticsStreamInputBlobReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &streamv1alpha1.AnalyticsStreamInputBlob{}); err != nil {
			klog.Error(err, "unable to set up auditor", streamv1alpha1.AnalyticsStreamInputBlob{}.APIVersion, streamv1alpha1.AnalyticsStreamInputBlob{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&streamv1alpha1.AnalyticsStreamInputBlob{}).
...
func (r *AnalyticsStreamInputBlobReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for AnalyticsStreamInputBlob")
			return nil
		}
		...
		resourceList := &streamv1alpha1.AnalyticsStreamInputBlobList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AnalyticsStreamInputBlob")
			return nil
		}
