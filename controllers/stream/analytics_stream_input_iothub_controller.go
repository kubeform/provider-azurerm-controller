...
// AnalyticsStreamInputIothubReconciler reconciles a AnalyticsStreamInputIothub object
type AnalyticsStreamInputIothubReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsstreaminputiothubs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsstreaminputiothubs/status,verbs=get;update;patch

func (r *AnalyticsStreamInputIothubReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyticsstreaminputiothub", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyticsStreamInputIothub")
...
func (r *AnalyticsStreamInputIothubReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &streamv1alpha1.AnalyticsStreamInputIothub{}); err != nil {
			klog.Error(err, "unable to set up auditor", streamv1alpha1.AnalyticsStreamInputIothub{}.APIVersion, streamv1alpha1.AnalyticsStreamInputIothub{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&streamv1alpha1.AnalyticsStreamInputIothub{}).
...
func (r *AnalyticsStreamInputIothubReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for AnalyticsStreamInputIothub")
			return nil
		}
		...
		resourceList := &streamv1alpha1.AnalyticsStreamInputIothubList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AnalyticsStreamInputIothub")
			return nil
		}
