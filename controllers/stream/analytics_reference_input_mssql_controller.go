...
// AnalyticsReferenceInputMssqlReconciler reconciles a AnalyticsReferenceInputMssql object
type AnalyticsReferenceInputMssqlReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsreferenceinputmssqls,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsreferenceinputmssqls/status,verbs=get;update;patch

func (r *AnalyticsReferenceInputMssqlReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyticsreferenceinputmssql", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyticsReferenceInputMssql")
...
func (r *AnalyticsReferenceInputMssqlReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &streamv1alpha1.AnalyticsReferenceInputMssql{}); err != nil {
			klog.Error(err, "unable to set up auditor", streamv1alpha1.AnalyticsReferenceInputMssql{}.APIVersion, streamv1alpha1.AnalyticsReferenceInputMssql{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&streamv1alpha1.AnalyticsReferenceInputMssql{}).
...
func (r *AnalyticsReferenceInputMssqlReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for AnalyticsReferenceInputMssql")
			return nil
		}
		...
		resourceList := &streamv1alpha1.AnalyticsReferenceInputMssqlList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AnalyticsReferenceInputMssql")
			return nil
		}
