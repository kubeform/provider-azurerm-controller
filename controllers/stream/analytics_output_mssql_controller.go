...
type AnalyticsOutputMssqlReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsoutputmssqls/status,verbs=get;update;patch
...
func (r *AnalyticsOutputMssqlReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyticsOutputMssql")
...
func (r *AnalyticsOutputMssqlReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&streamv1alpha1.AnalyticsOutputMssql{}).
...
func (r *AnalyticsOutputMssqlReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for AnalyticsOutputMssql")
			return nil
		}
