...
// AnalyticsOutputSynapseReconciler reconciles a AnalyticsOutputSynapse object
type AnalyticsOutputSynapseReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsoutputsynapsis,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsoutputsynapsis/status,verbs=get;update;patch

func (r *AnalyticsOutputSynapseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyticsoutputsynapse", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyticsOutputSynapse")
...
func (r *AnalyticsOutputSynapseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &streamv1alpha1.AnalyticsOutputSynapse{}); err != nil {
			klog.Error(err, "unable to set up auditor", streamv1alpha1.AnalyticsOutputSynapse{}.APIVersion, streamv1alpha1.AnalyticsOutputSynapse{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&streamv1alpha1.AnalyticsOutputSynapse{}).
...
func (r *AnalyticsOutputSynapseReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for AnalyticsOutputSynapse")
			return nil
		}
		...
		resourceList := &streamv1alpha1.AnalyticsOutputSynapseList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AnalyticsOutputSynapse")
			return nil
		}
