...
// AnalyticsFunctionJavascriptUdfReconciler reconciles a AnalyticsFunctionJavascriptUdf object
type AnalyticsFunctionJavascriptUdfReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsfunctionjavascriptudfs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=stream.azurerm.kubeform.com,resources=analyticsfunctionjavascriptudfs/status,verbs=get;update;patch

func (r *AnalyticsFunctionJavascriptUdfReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyticsfunctionjavascriptudf", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyticsFunctionJavascriptUdf")
...
func (r *AnalyticsFunctionJavascriptUdfReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &streamv1alpha1.AnalyticsFunctionJavascriptUdf{}); err != nil {
			klog.Error(err, "unable to set up auditor", streamv1alpha1.AnalyticsFunctionJavascriptUdf{}.APIVersion, streamv1alpha1.AnalyticsFunctionJavascriptUdf{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&streamv1alpha1.AnalyticsFunctionJavascriptUdf{}).
