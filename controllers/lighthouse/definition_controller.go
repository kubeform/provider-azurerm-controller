...
// +kubebuilder:rbac:groups=lighthouse.azurerm.kubeform.com,resources=definitions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lighthouse.azurerm.kubeform.com,resources=definitions/status,verbs=get;update;patch

func (r *DefinitionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("definition", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Definition")
...
func (r *DefinitionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lighthousev1alpha1.Definition{}); err != nil {
			klog.Error(err, "unable to set up auditor", lighthousev1alpha1.Definition{}.APIVersion, lighthousev1alpha1.Definition{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lighthousev1alpha1.Definition{}).
