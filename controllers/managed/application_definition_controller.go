...
// ApplicationDefinitionReconciler reconciles a ApplicationDefinition object
...
// +kubebuilder:rbac:groups=managed.azurerm.kubeform.com,resources=applicationdefinitions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=managed.azurerm.kubeform.com,resources=applicationdefinitions/status,verbs=get;update;patch

func (r *ApplicationDefinitionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("applicationdefinition", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApplicationDefinition")
...
func (r *ApplicationDefinitionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &managedv1alpha1.ApplicationDefinition{}); err != nil {
			klog.Error(err, "unable to set up auditor", managedv1alpha1.ApplicationDefinition{}.APIVersion, managedv1alpha1.ApplicationDefinition{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&managedv1alpha1.ApplicationDefinition{}).
