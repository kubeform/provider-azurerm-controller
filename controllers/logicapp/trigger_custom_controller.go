...
// TriggerCustomReconciler reconciles a TriggerCustom object
type TriggerCustomReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logicapp.azurerm.kubeform.com,resources=triggercustoms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logicapp.azurerm.kubeform.com,resources=triggercustoms/status,verbs=get;update;patch

func (r *TriggerCustomReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("triggercustom", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TriggerCustom")
...
func (r *TriggerCustomReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logicappv1alpha1.TriggerCustom{}); err != nil {
			klog.Error(err, "unable to set up auditor", logicappv1alpha1.TriggerCustom{}.APIVersion, logicappv1alpha1.TriggerCustom{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logicappv1alpha1.TriggerCustom{}).
