...
// IntegrationAccountAssemblyReconciler reconciles a IntegrationAccountAssembly object
type IntegrationAccountAssemblyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logicapp.azurerm.kubeform.com,resources=integrationaccountassemblies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logicapp.azurerm.kubeform.com,resources=integrationaccountassemblies/status,verbs=get;update;patch

func (r *IntegrationAccountAssemblyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("integrationaccountassembly", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IntegrationAccountAssembly")
...
func (r *IntegrationAccountAssemblyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logicappv1alpha1.IntegrationAccountAssembly{}); err != nil {
			klog.Error(err, "unable to set up auditor", logicappv1alpha1.IntegrationAccountAssembly{}.APIVersion, logicappv1alpha1.IntegrationAccountAssembly{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logicappv1alpha1.IntegrationAccountAssembly{}).
