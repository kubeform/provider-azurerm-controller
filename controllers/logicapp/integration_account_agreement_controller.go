...
// IntegrationAccountAgreementReconciler reconciles a IntegrationAccountAgreement object
type IntegrationAccountAgreementReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logicapp.azurerm.kubeform.com,resources=integrationaccountagreements,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logicapp.azurerm.kubeform.com,resources=integrationaccountagreements/status,verbs=get;update;patch

func (r *IntegrationAccountAgreementReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("integrationaccountagreement", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IntegrationAccountAgreement")
...
func (r *IntegrationAccountAgreementReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logicappv1alpha1.IntegrationAccountAgreement{}); err != nil {
			klog.Error(err, "unable to set up auditor", logicappv1alpha1.IntegrationAccountAgreement{}.APIVersion, logicappv1alpha1.IntegrationAccountAgreement{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logicappv1alpha1.IntegrationAccountAgreement{}).
