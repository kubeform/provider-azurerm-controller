...
// ActionCustomReconciler reconciles a ActionCustom object
type ActionCustomReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logicapp.azurerm.kubeform.com,resources=actioncustoms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logicapp.azurerm.kubeform.com,resources=actioncustoms/status,verbs=get;update;patch

func (r *ActionCustomReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("actioncustom", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ActionCustom")
...
func (r *ActionCustomReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logicappv1alpha1.ActionCustom{}); err != nil {
			klog.Error(err, "unable to set up auditor", logicappv1alpha1.ActionCustom{}.APIVersion, logicappv1alpha1.ActionCustom{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logicappv1alpha1.ActionCustom{}).
