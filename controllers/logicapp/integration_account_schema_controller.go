...
// IntegrationAccountSchemaReconciler reconciles a IntegrationAccountSchema object
type IntegrationAccountSchemaReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logicapp.azurerm.kubeform.com,resources=integrationaccountschemas,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logicapp.azurerm.kubeform.com,resources=integrationaccountschemas/status,verbs=get;update;patch

func (r *IntegrationAccountSchemaReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("integrationaccountschema", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IntegrationAccountSchema")
...
func (r *IntegrationAccountSchemaReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logicappv1alpha1.IntegrationAccountSchema{}); err != nil {
			klog.Error(err, "unable to set up auditor", logicappv1alpha1.IntegrationAccountSchema{}.APIVersion, logicappv1alpha1.IntegrationAccountSchema{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logicappv1alpha1.IntegrationAccountSchema{}).
