...
// IntegrationAccountCertificateReconciler reconciles a IntegrationAccountCertificate object
type IntegrationAccountCertificateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logicapp.azurerm.kubeform.com,resources=integrationaccountcertificates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logicapp.azurerm.kubeform.com,resources=integrationaccountcertificates/status,verbs=get;update;patch

func (r *IntegrationAccountCertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("integrationaccountcertificate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IntegrationAccountCertificate")
...
func (r *IntegrationAccountCertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logicappv1alpha1.IntegrationAccountCertificate{}); err != nil {
			klog.Error(err, "unable to set up auditor", logicappv1alpha1.IntegrationAccountCertificate{}.APIVersion, logicappv1alpha1.IntegrationAccountCertificate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logicappv1alpha1.IntegrationAccountCertificate{}).
