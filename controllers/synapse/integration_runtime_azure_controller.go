...
// IntegrationRuntimeAzureReconciler reconciles a IntegrationRuntimeAzure object
type IntegrationRuntimeAzureReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=integrationruntimeazures,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=integrationruntimeazures/status,verbs=get;update;patch

func (r *IntegrationRuntimeAzureReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("integrationruntimeazure", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IntegrationRuntimeAzure")
...
func (r *IntegrationRuntimeAzureReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &synapsev1alpha1.IntegrationRuntimeAzure{}); err != nil {
			klog.Error(err, "unable to set up auditor", synapsev1alpha1.IntegrationRuntimeAzure{}.APIVersion, synapsev1alpha1.IntegrationRuntimeAzure{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&synapsev1alpha1.IntegrationRuntimeAzure{}).
