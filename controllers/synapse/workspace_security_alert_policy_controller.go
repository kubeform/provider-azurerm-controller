...
// WorkspaceSecurityAlertPolicyReconciler reconciles a WorkspaceSecurityAlertPolicy object
type WorkspaceSecurityAlertPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=workspacesecurityalertpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=workspacesecurityalertpolicies/status,verbs=get;update;patch

func (r *WorkspaceSecurityAlertPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("workspacesecurityalertpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WorkspaceSecurityAlertPolicy")
...
func (r *WorkspaceSecurityAlertPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &synapsev1alpha1.WorkspaceSecurityAlertPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", synapsev1alpha1.WorkspaceSecurityAlertPolicy{}.APIVersion, synapsev1alpha1.WorkspaceSecurityAlertPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&synapsev1alpha1.WorkspaceSecurityAlertPolicy{}).
...
func (r *WorkspaceSecurityAlertPolicyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for WorkspaceSecurityAlertPolicy")
			return nil
		}
		...
		resourceList := &synapsev1alpha1.WorkspaceSecurityAlertPolicyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list WorkspaceSecurityAlertPolicy")
			return nil
		}
