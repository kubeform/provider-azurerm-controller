...
// IntegrationRuntimeSelfHostedReconciler reconciles a IntegrationRuntimeSelfHosted object
type IntegrationRuntimeSelfHostedReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=integrationruntimeselfhosteds,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=integrationruntimeselfhosteds/status,verbs=get;update;patch

func (r *IntegrationRuntimeSelfHostedReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("integrationruntimeselfhosted", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IntegrationRuntimeSelfHosted")
...
func (r *IntegrationRuntimeSelfHostedReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &synapsev1alpha1.IntegrationRuntimeSelfHosted{}); err != nil {
			klog.Error(err, "unable to set up auditor", synapsev1alpha1.IntegrationRuntimeSelfHosted{}.APIVersion, synapsev1alpha1.IntegrationRuntimeSelfHosted{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&synapsev1alpha1.IntegrationRuntimeSelfHosted{}).
