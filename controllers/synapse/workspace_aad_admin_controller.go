...
// WorkspaceAadAdminReconciler reconciles a WorkspaceAadAdmin object
type WorkspaceAadAdminReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=workspaceaadadmins,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=workspaceaadadmins/status,verbs=get;update;patch

func (r *WorkspaceAadAdminReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("workspaceaadadmin", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WorkspaceAadAdmin")
...
func (r *WorkspaceAadAdminReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &synapsev1alpha1.WorkspaceAadAdmin{}); err != nil {
			klog.Error(err, "unable to set up auditor", synapsev1alpha1.WorkspaceAadAdmin{}.APIVersion, synapsev1alpha1.WorkspaceAadAdmin{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&synapsev1alpha1.WorkspaceAadAdmin{}).
