...
// SqlPoolWorkloadClassifierReconciler reconciles a SqlPoolWorkloadClassifier object
type SqlPoolWorkloadClassifierReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=sqlpoolworkloadclassifiers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=sqlpoolworkloadclassifiers/status,verbs=get;update;patch

func (r *SqlPoolWorkloadClassifierReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sqlpoolworkloadclassifier", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SqlPoolWorkloadClassifier")
...
func (r *SqlPoolWorkloadClassifierReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &synapsev1alpha1.SqlPoolWorkloadClassifier{}); err != nil {
			klog.Error(err, "unable to set up auditor", synapsev1alpha1.SqlPoolWorkloadClassifier{}.APIVersion, synapsev1alpha1.SqlPoolWorkloadClassifier{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&synapsev1alpha1.SqlPoolWorkloadClassifier{}).
