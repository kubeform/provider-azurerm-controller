...
// ManagedPrivateEndpointReconciler reconciles a ManagedPrivateEndpoint object
type ManagedPrivateEndpointReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=managedprivateendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=managedprivateendpoints/status,verbs=get;update;patch

func (r *ManagedPrivateEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managedprivateendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagedPrivateEndpoint")
...
func (r *ManagedPrivateEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &synapsev1alpha1.ManagedPrivateEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", synapsev1alpha1.ManagedPrivateEndpoint{}.APIVersion, synapsev1alpha1.ManagedPrivateEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&synapsev1alpha1.ManagedPrivateEndpoint{}).
