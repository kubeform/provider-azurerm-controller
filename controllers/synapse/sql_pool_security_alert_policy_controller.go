...
// SqlPoolSecurityAlertPolicyReconciler reconciles a SqlPoolSecurityAlertPolicy object
type SqlPoolSecurityAlertPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=sqlpoolsecurityalertpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=sqlpoolsecurityalertpolicies/status,verbs=get;update;patch

func (r *SqlPoolSecurityAlertPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sqlpoolsecurityalertpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SqlPoolSecurityAlertPolicy")
...
func (r *SqlPoolSecurityAlertPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &synapsev1alpha1.SqlPoolSecurityAlertPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", synapsev1alpha1.SqlPoolSecurityAlertPolicy{}.APIVersion, synapsev1alpha1.SqlPoolSecurityAlertPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&synapsev1alpha1.SqlPoolSecurityAlertPolicy{}).
...
func (r *SqlPoolSecurityAlertPolicyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for SqlPoolSecurityAlertPolicy")
			return nil
		}
		...
		resourceList := &synapsev1alpha1.SqlPoolSecurityAlertPolicyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SqlPoolSecurityAlertPolicy")
			return nil
		}
