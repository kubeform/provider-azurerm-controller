...
// SqlPoolWorkloadGroupReconciler reconciles a SqlPoolWorkloadGroup object
type SqlPoolWorkloadGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=sqlpoolworkloadgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synapse.azurerm.kubeform.com,resources=sqlpoolworkloadgroups/status,verbs=get;update;patch

func (r *SqlPoolWorkloadGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sqlpoolworkloadgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SqlPoolWorkloadGroup")
...
func (r *SqlPoolWorkloadGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &synapsev1alpha1.SqlPoolWorkloadGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", synapsev1alpha1.SqlPoolWorkloadGroup{}.APIVersion, synapsev1alpha1.SqlPoolWorkloadGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&synapsev1alpha1.SqlPoolWorkloadGroup{}).
