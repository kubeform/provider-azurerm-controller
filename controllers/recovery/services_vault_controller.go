...
// ServicesVaultReconciler reconciles a ServicesVault object
type ServicesVaultReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=recovery.azurerm.kubeform.com,resources=servicesvaults,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=recovery.azurerm.kubeform.com,resources=servicesvaults/status,verbs=get;update;patch

func (r *ServicesVaultReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicesvault", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServicesVault")
...
func (r *ServicesVaultReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &recoveryv1alpha1.ServicesVault{}); err != nil {
			klog.Error(err, "unable to set up auditor", recoveryv1alpha1.ServicesVault{}.APIVersion, recoveryv1alpha1.ServicesVault{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&recoveryv1alpha1.ServicesVault{}).
