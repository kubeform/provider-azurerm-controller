...
// ChannelSlackReconciler reconciles a ChannelSlack object
type ChannelSlackReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=bot.azurerm.kubeform.com,resources=channelslacks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bot.azurerm.kubeform.com,resources=channelslacks/status,verbs=get;update;patch

func (r *ChannelSlackReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("channelslack", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ChannelSlack")
...
func (r *ChannelSlackReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &botv1alpha1.ChannelSlack{}); err != nil {
			klog.Error(err, "unable to set up auditor", botv1alpha1.ChannelSlack{}.APIVersion, botv1alpha1.ChannelSlack{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&botv1alpha1.ChannelSlack{}).
...
func (r *ChannelSlackReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ChannelSlack")
			return nil
		}
		...
		resourceList := &botv1alpha1.ChannelSlackList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ChannelSlack")
			return nil
		}
