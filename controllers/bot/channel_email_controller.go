...
// ChannelEmailReconciler reconciles a ChannelEmail object
type ChannelEmailReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=bot.azurerm.kubeform.com,resources=channelemails,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bot.azurerm.kubeform.com,resources=channelemails/status,verbs=get;update;patch

func (r *ChannelEmailReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("channelemail", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ChannelEmail")
...
func (r *ChannelEmailReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &botv1alpha1.ChannelEmail{}); err != nil {
			klog.Error(err, "unable to set up auditor", botv1alpha1.ChannelEmail{}.APIVersion, botv1alpha1.ChannelEmail{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&botv1alpha1.ChannelEmail{}).
...
func (r *ChannelEmailReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ChannelEmail")
			return nil
		}
		...
		resourceList := &botv1alpha1.ChannelEmailList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ChannelEmail")
			return nil
		}
