...
// ChannelDirectLineSpeechReconciler reconciles a ChannelDirectLineSpeech object
type ChannelDirectLineSpeechReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=bot.azurerm.kubeform.com,resources=channeldirectlinespeeches,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bot.azurerm.kubeform.com,resources=channeldirectlinespeeches/status,verbs=get;update;patch

func (r *ChannelDirectLineSpeechReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("channeldirectlinespeech", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ChannelDirectLineSpeech")
...
func (r *ChannelDirectLineSpeechReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &botv1alpha1.ChannelDirectLineSpeech{}); err != nil {
			klog.Error(err, "unable to set up auditor", botv1alpha1.ChannelDirectLineSpeech{}.APIVersion, botv1alpha1.ChannelDirectLineSpeech{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&botv1alpha1.ChannelDirectLineSpeech{}).
...
func (r *ChannelDirectLineSpeechReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for ChannelDirectLineSpeech")
			return nil
		}
		...
		resourceList := &botv1alpha1.ChannelDirectLineSpeechList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ChannelDirectLineSpeech")
			return nil
		}
