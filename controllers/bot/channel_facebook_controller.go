...
// ChannelFacebookReconciler reconciles a ChannelFacebook object
type ChannelFacebookReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=bot.azurerm.kubeform.com,resources=channelfacebooks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bot.azurerm.kubeform.com,resources=channelfacebooks/status,verbs=get;update;patch

func (r *ChannelFacebookReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("channelfacebook", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ChannelFacebook")
...
func (r *ChannelFacebookReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &botv1alpha1.ChannelFacebook{}); err != nil {
			klog.Error(err, "unable to set up auditor", botv1alpha1.ChannelFacebook{}.APIVersion, botv1alpha1.ChannelFacebook{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&botv1alpha1.ChannelFacebook{}).
...
func (r *ChannelFacebookReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for ChannelFacebook")
			return nil
		}
		...
		resourceList := &botv1alpha1.ChannelFacebookList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ChannelFacebook")
			return nil
		}
