...
// ChannelMsTeamsReconciler reconciles a ChannelMsTeams object
type ChannelMsTeamsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=bot.azurerm.kubeform.com,resources=channelmsteams,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bot.azurerm.kubeform.com,resources=channelmsteams/status,verbs=get;update;patch

func (r *ChannelMsTeamsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("channelmsteams", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ChannelMsTeams")
...
func (r *ChannelMsTeamsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &botv1alpha1.ChannelMsTeams{}); err != nil {
			klog.Error(err, "unable to set up auditor", botv1alpha1.ChannelMsTeams{}.APIVersion, botv1alpha1.ChannelMsTeams{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&botv1alpha1.ChannelMsTeams{}).
