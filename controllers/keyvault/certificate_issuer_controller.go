...
// CertificateIssuerReconciler reconciles a CertificateIssuer object
type CertificateIssuerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=keyvault.azurerm.kubeform.com,resources=certificateissuers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=keyvault.azurerm.kubeform.com,resources=certificateissuers/status,verbs=get;update;patch

func (r *CertificateIssuerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("certificateissuer", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CertificateIssuer")
...
func (r *CertificateIssuerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &keyvaultv1alpha1.CertificateIssuer{}); err != nil {
			klog.Error(err, "unable to set up auditor", keyvaultv1alpha1.CertificateIssuer{}.APIVersion, keyvaultv1alpha1.CertificateIssuer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&keyvaultv1alpha1.CertificateIssuer{}).
...
func (r *CertificateIssuerReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for CertificateIssuer")
			return nil
		}
		...
		resourceList := &keyvaultv1alpha1.CertificateIssuerList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list CertificateIssuer")
			return nil
		}
