...
// ManagedStorageAccountReconciler reconciles a ManagedStorageAccount object
type ManagedStorageAccountReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=keyvault.azurerm.kubeform.com,resources=managedstorageaccounts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=keyvault.azurerm.kubeform.com,resources=managedstorageaccounts/status,verbs=get;update;patch

func (r *ManagedStorageAccountReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managedstorageaccount", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagedStorageAccount")
...
func (r *ManagedStorageAccountReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &keyvaultv1alpha1.ManagedStorageAccount{}); err != nil {
			klog.Error(err, "unable to set up auditor", keyvaultv1alpha1.ManagedStorageAccount{}.APIVersion, keyvaultv1alpha1.ManagedStorageAccount{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&keyvaultv1alpha1.ManagedStorageAccount{}).
