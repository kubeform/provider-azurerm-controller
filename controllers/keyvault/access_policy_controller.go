...
// +kubebuilder:rbac:groups=keyvault.azurerm.kubeform.com,resources=accesspolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=keyvault.azurerm.kubeform.com,resources=accesspolicies/status,verbs=get;update;patch
...
func (r *AccessPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccessPolicy")
...
func (r *AccessPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &keyvaultv1alpha1.AccessPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", keyvaultv1alpha1.AccessPolicy{}.APIVersion, keyvaultv1alpha1.AccessPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&keyvaultv1alpha1.AccessPolicy{}).
