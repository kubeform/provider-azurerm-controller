...
// SystemTopicEventSubscriptionReconciler reconciles a SystemTopicEventSubscription object
type SystemTopicEventSubscriptionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=eventgrid.azurerm.kubeform.com,resources=systemtopiceventsubscriptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=eventgrid.azurerm.kubeform.com,resources=systemtopiceventsubscriptions/status,verbs=get;update;patch

func (r *SystemTopicEventSubscriptionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("systemtopiceventsubscription", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SystemTopicEventSubscription")
...
func (r *SystemTopicEventSubscriptionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eventgridv1alpha1.SystemTopicEventSubscription{}); err != nil {
			klog.Error(err, "unable to set up auditor", eventgridv1alpha1.SystemTopicEventSubscription{}.APIVersion, eventgridv1alpha1.SystemTopicEventSubscription{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eventgridv1alpha1.SystemTopicEventSubscription{}).
