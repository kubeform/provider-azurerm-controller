...
// DomainTopicReconciler reconciles a DomainTopic object
type DomainTopicReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=eventgrid.azurerm.kubeform.com,resources=domaintopics,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=eventgrid.azurerm.kubeform.com,resources=domaintopics/status,verbs=get;update;patch

func (r *DomainTopicReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("domaintopic", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DomainTopic")
...
func (r *DomainTopicReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eventgridv1alpha1.DomainTopic{}); err != nil {
			klog.Error(err, "unable to set up auditor", eventgridv1alpha1.DomainTopic{}.APIVersion, eventgridv1alpha1.DomainTopic{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eventgridv1alpha1.DomainTopic{}).
