...
// VariableDatetimeReconciler reconciles a VariableDatetime object
type VariableDatetimeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=automation.azurerm.kubeform.com,resources=variabledatetimes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=automation.azurerm.kubeform.com,resources=variabledatetimes/status,verbs=get;update;patch

func (r *VariableDatetimeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("variabledatetime", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VariableDatetime")
...
func (r *VariableDatetimeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &automationv1alpha1.VariableDatetime{}); err != nil {
			klog.Error(err, "unable to set up auditor", automationv1alpha1.VariableDatetime{}.APIVersion, automationv1alpha1.VariableDatetime{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&automationv1alpha1.VariableDatetime{}).
