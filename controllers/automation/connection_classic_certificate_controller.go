...
// ConnectionClassicCertificateReconciler reconciles a ConnectionClassicCertificate object
type ConnectionClassicCertificateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=automation.azurerm.kubeform.com,resources=connectionclassiccertificates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=automation.azurerm.kubeform.com,resources=connectionclassiccertificates/status,verbs=get;update;patch

func (r *ConnectionClassicCertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("connectionclassiccertificate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectionClassicCertificate")
...
func (r *ConnectionClassicCertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &automationv1alpha1.ConnectionClassicCertificate{}); err != nil {
			klog.Error(err, "unable to set up auditor", automationv1alpha1.ConnectionClassicCertificate{}.APIVersion, automationv1alpha1.ConnectionClassicCertificate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&automationv1alpha1.ConnectionClassicCertificate{}).
