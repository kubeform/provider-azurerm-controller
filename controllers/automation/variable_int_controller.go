...
// VariableIntReconciler reconciles a VariableInt object
type VariableIntReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=automation.azurerm.kubeform.com,resources=variableints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=automation.azurerm.kubeform.com,resources=variableints/status,verbs=get;update;patch

func (r *VariableIntReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("variableint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VariableInt")
...
func (r *VariableIntReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &automationv1alpha1.VariableInt{}); err != nil {
			klog.Error(err, "unable to set up auditor", automationv1alpha1.VariableInt{}.APIVersion, automationv1alpha1.VariableInt{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&automationv1alpha1.VariableInt{}).
