...
// ConnectionServicePrincipalReconciler reconciles a ConnectionServicePrincipal object
type ConnectionServicePrincipalReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=automation.azurerm.kubeform.com,resources=connectionserviceprincipals/status,verbs=get;update;patch
...
func (r *ConnectionServicePrincipalReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectionServicePrincipal")
...
func (r *ConnectionServicePrincipalReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
