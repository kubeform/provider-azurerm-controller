...
// DscNodeconfigurationReconciler reconciles a DscNodeconfiguration object
type DscNodeconfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=automation.azurerm.kubeform.com,resources=dscnodeconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=automation.azurerm.kubeform.com,resources=dscnodeconfigurations/status,verbs=get;update;patch

func (r *DscNodeconfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dscnodeconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DscNodeconfiguration")
...
func (r *DscNodeconfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &automationv1alpha1.DscNodeconfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", automationv1alpha1.DscNodeconfiguration{}.APIVersion, automationv1alpha1.DscNodeconfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&automationv1alpha1.DscNodeconfiguration{}).
