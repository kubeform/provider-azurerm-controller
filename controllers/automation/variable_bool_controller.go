...
// VariableBoolReconciler reconciles a VariableBool object
type VariableBoolReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=automation.azurerm.kubeform.com,resources=variablebools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=automation.azurerm.kubeform.com,resources=variablebools/status,verbs=get;update;patch

func (r *VariableBoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("variablebool", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VariableBool")
...
func (r *VariableBoolReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &automationv1alpha1.VariableBool{}); err != nil {
			klog.Error(err, "unable to set up auditor", automationv1alpha1.VariableBool{}.APIVersion, automationv1alpha1.VariableBool{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&automationv1alpha1.VariableBool{}).
