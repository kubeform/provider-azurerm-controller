...
// CacheBlobTargetReconciler reconciles a CacheBlobTarget object
type CacheBlobTargetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=hpc.azurerm.kubeform.com,resources=cacheblobtargets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hpc.azurerm.kubeform.com,resources=cacheblobtargets/status,verbs=get;update;patch

func (r *CacheBlobTargetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cacheblobtarget", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CacheBlobTarget")
...
func (r *CacheBlobTargetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hpcv1alpha1.CacheBlobTarget{}); err != nil {
			klog.Error(err, "unable to set up auditor", hpcv1alpha1.CacheBlobTarget{}.APIVersion, hpcv1alpha1.CacheBlobTarget{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hpcv1alpha1.CacheBlobTarget{}).
