...
// CacheAccessPolicyReconciler reconciles a CacheAccessPolicy object
type CacheAccessPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=hpc.azurerm.kubeform.com,resources=cacheaccesspolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hpc.azurerm.kubeform.com,resources=cacheaccesspolicies/status,verbs=get;update;patch

func (r *CacheAccessPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cacheaccesspolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CacheAccessPolicy")
...
func (r *CacheAccessPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hpcv1alpha1.CacheAccessPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", hpcv1alpha1.CacheAccessPolicy{}.APIVersion, hpcv1alpha1.CacheAccessPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hpcv1alpha1.CacheAccessPolicy{}).
