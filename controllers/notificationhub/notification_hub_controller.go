...
// NotificationHubReconciler reconciles a NotificationHub object
type NotificationHubReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=notificationhub.azurerm.kubeform.com,resources=notificationhubs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=notificationhub.azurerm.kubeform.com,resources=notificationhubs/status,verbs=get;update;patch

func (r *NotificationHubReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("notificationhub", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NotificationHub")
...
func (r *NotificationHubReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &notificationhubv1alpha1.NotificationHub{}); err != nil {
			klog.Error(err, "unable to set up auditor", notificationhubv1alpha1.NotificationHub{}.APIVersion, notificationhubv1alpha1.NotificationHub{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&notificationhubv1alpha1.NotificationHub{}).
...
func (r *NotificationHubReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for NotificationHub")
			return nil
		}
		...
		resourceList := &notificationhubv1alpha1.NotificationHubList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list NotificationHub")
			return nil
		}
