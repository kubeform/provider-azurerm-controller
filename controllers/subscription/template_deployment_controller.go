...
// TemplateDeploymentReconciler reconciles a TemplateDeployment object
type TemplateDeploymentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=subscription.azurerm.kubeform.com,resources=templatedeployments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=subscription.azurerm.kubeform.com,resources=templatedeployments/status,verbs=get;update;patch

func (r *TemplateDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("templatedeployment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TemplateDeployment")
...
func (r *TemplateDeploymentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &subscriptionv1alpha1.TemplateDeployment{}); err != nil {
			klog.Error(err, "unable to set up auditor", subscriptionv1alpha1.TemplateDeployment{}.APIVersion, subscriptionv1alpha1.TemplateDeployment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&subscriptionv1alpha1.TemplateDeployment{}).
