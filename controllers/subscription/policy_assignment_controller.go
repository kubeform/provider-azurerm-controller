...
type PolicyAssignmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=subscription.azurerm.kubeform.com,resources=policyassignments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=subscription.azurerm.kubeform.com,resources=policyassignments/status,verbs=get;update;patch

func (r *PolicyAssignmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("policyassignment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PolicyAssignment")
...
func (r *PolicyAssignmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &subscriptionv1alpha1.PolicyAssignment{}); err != nil {
			klog.Error(err, "unable to set up auditor", subscriptionv1alpha1.PolicyAssignment{}.APIVersion, subscriptionv1alpha1.PolicyAssignment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&subscriptionv1alpha1.PolicyAssignment{}).
