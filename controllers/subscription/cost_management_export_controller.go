...
// CostManagementExportReconciler reconciles a CostManagementExport object
type CostManagementExportReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=subscription.azurerm.kubeform.com,resources=costmanagementexports,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=subscription.azurerm.kubeform.com,resources=costmanagementexports/status,verbs=get;update;patch

func (r *CostManagementExportReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("costmanagementexport", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CostManagementExport")
...
func (r *CostManagementExportReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &subscriptionv1alpha1.CostManagementExport{}); err != nil {
			klog.Error(err, "unable to set up auditor", subscriptionv1alpha1.CostManagementExport{}.APIVersion, subscriptionv1alpha1.CostManagementExport{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&subscriptionv1alpha1.CostManagementExport{}).
