...
// ToSiteVPNGatewayReconciler reconciles a ToSiteVPNGateway object
type ToSiteVPNGatewayReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=point.azurerm.kubeform.com,resources=tositevpngateways,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=point.azurerm.kubeform.com,resources=tositevpngateways/status,verbs=get;update;patch

func (r *ToSiteVPNGatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tositevpngateway", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ToSiteVPNGateway")
...
func (r *ToSiteVPNGatewayReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &pointv1alpha1.ToSiteVPNGateway{}); err != nil {
			klog.Error(err, "unable to set up auditor", pointv1alpha1.ToSiteVPNGateway{}.APIVersion, pointv1alpha1.ToSiteVPNGateway{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&pointv1alpha1.ToSiteVPNGateway{}).
