...
// ProtectionContainerMappingReconciler reconciles a ProtectionContainerMapping object
type ProtectionContainerMappingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=siterecovery.azurerm.kubeform.com,resources=protectioncontainermappings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=siterecovery.azurerm.kubeform.com,resources=protectioncontainermappings/status,verbs=get;update;patch

func (r *ProtectionContainerMappingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("protectioncontainermapping", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProtectionContainerMapping")
...
func (r *ProtectionContainerMappingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &siterecoveryv1alpha1.ProtectionContainerMapping{}); err != nil {
			klog.Error(err, "unable to set up auditor", siterecoveryv1alpha1.ProtectionContainerMapping{}.APIVersion, siterecoveryv1alpha1.ProtectionContainerMapping{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&siterecoveryv1alpha1.ProtectionContainerMapping{}).
