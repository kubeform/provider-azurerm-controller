...
// +kubebuilder:rbac:groups=siterecovery.azurerm.kubeform.com,resources=networkmappings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=siterecovery.azurerm.kubeform.com,resources=networkmappings/status,verbs=get;update;patch

func (r *NetworkMappingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networkmapping", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkMapping")
...
func (r *NetworkMappingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &siterecoveryv1alpha1.NetworkMapping{}); err != nil {
			klog.Error(err, "unable to set up auditor", siterecoveryv1alpha1.NetworkMapping{}.APIVersion, siterecoveryv1alpha1.NetworkMapping{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&siterecoveryv1alpha1.NetworkMapping{}).
