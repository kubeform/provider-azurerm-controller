...
// ReplicatedVmReconciler reconciles a ReplicatedVm object
type ReplicatedVmReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=siterecovery.azurerm.kubeform.com,resources=replicatedvms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=siterecovery.azurerm.kubeform.com,resources=replicatedvms/status,verbs=get;update;patch

func (r *ReplicatedVmReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("replicatedvm", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReplicatedVm")
...
func (r *ReplicatedVmReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &siterecoveryv1alpha1.ReplicatedVm{}); err != nil {
			klog.Error(err, "unable to set up auditor", siterecoveryv1alpha1.ReplicatedVm{}.APIVersion, siterecoveryv1alpha1.ReplicatedVm{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&siterecoveryv1alpha1.ReplicatedVm{}).
