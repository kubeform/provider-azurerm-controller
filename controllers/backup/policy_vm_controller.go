...
// PolicyVmReconciler reconciles a PolicyVm object
type PolicyVmReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=backup.azurerm.kubeform.com,resources=policyvms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=backup.azurerm.kubeform.com,resources=policyvms/status,verbs=get;update;patch

func (r *PolicyVmReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("policyvm", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PolicyVm")
...
func (r *PolicyVmReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &backupv1alpha1.PolicyVm{}); err != nil {
			klog.Error(err, "unable to set up auditor", backupv1alpha1.PolicyVm{}.APIVersion, backupv1alpha1.PolicyVm{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&backupv1alpha1.PolicyVm{}).
