...
// ContainerStorageAccountReconciler reconciles a ContainerStorageAccount object
type ContainerStorageAccountReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=backup.azurerm.kubeform.com,resources=containerstorageaccounts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=backup.azurerm.kubeform.com,resources=containerstorageaccounts/status,verbs=get;update;patch

func (r *ContainerStorageAccountReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("containerstorageaccount", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ContainerStorageAccount")
...
func (r *ContainerStorageAccountReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &backupv1alpha1.ContainerStorageAccount{}); err != nil {
			klog.Error(err, "unable to set up auditor", backupv1alpha1.ContainerStorageAccount{}.APIVersion, backupv1alpha1.ContainerStorageAccount{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&backupv1alpha1.ContainerStorageAccount{}).
