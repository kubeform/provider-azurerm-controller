...
// ProtectedVmReconciler reconciles a ProtectedVm object
type ProtectedVmReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=backup.azurerm.kubeform.com,resources=protectedvms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=backup.azurerm.kubeform.com,resources=protectedvms/status,verbs=get;update;patch

func (r *ProtectedVmReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("protectedvm", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProtectedVm")
...
func (r *ProtectedVmReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &backupv1alpha1.ProtectedVm{}); err != nil {
			klog.Error(err, "unable to set up auditor", backupv1alpha1.ProtectedVm{}.APIVersion, backupv1alpha1.ProtectedVm{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&backupv1alpha1.ProtectedVm{}).
