...
// PolicyFileShareReconciler reconciles a PolicyFileShare object
type PolicyFileShareReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=backup.azurerm.kubeform.com,resources=policyfileshares,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=backup.azurerm.kubeform.com,resources=policyfileshares/status,verbs=get;update;patch

func (r *PolicyFileShareReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("policyfileshare", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PolicyFileShare")
...
func (r *PolicyFileShareReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &backupv1alpha1.PolicyFileShare{}); err != nil {
			klog.Error(err, "unable to set up auditor", backupv1alpha1.PolicyFileShare{}.APIVersion, backupv1alpha1.PolicyFileShare{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&backupv1alpha1.PolicyFileShare{}).
