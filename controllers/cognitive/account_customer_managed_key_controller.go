...
// AccountCustomerManagedKeyReconciler reconciles a AccountCustomerManagedKey object
type AccountCustomerManagedKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cognitive.azurerm.kubeform.com,resources=accountcustomermanagedkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cognitive.azurerm.kubeform.com,resources=accountcustomermanagedkeys/status,verbs=get;update;patch

func (r *AccountCustomerManagedKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accountcustomermanagedkey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccountCustomerManagedKey")
...
func (r *AccountCustomerManagedKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cognitivev1alpha1.AccountCustomerManagedKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", cognitivev1alpha1.AccountCustomerManagedKey{}.APIVersion, cognitivev1alpha1.AccountCustomerManagedKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cognitivev1alpha1.AccountCustomerManagedKey{}).
