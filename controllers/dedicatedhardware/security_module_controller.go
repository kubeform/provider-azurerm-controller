...
// SecurityModuleReconciler reconciles a SecurityModule object
type SecurityModuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dedicatedhardware.azurerm.kubeform.com,resources=securitymodules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dedicatedhardware.azurerm.kubeform.com,resources=securitymodules/status,verbs=get;update;patch

func (r *SecurityModuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("securitymodule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SecurityModule")
...
func (r *SecurityModuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dedicatedhardwarev1alpha1.SecurityModule{}); err != nil {
			klog.Error(err, "unable to set up auditor", dedicatedhardwarev1alpha1.SecurityModule{}.APIVersion, dedicatedhardwarev1alpha1.SecurityModule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dedicatedhardwarev1alpha1.SecurityModule{}).
