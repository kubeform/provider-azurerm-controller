...
// +kubebuilder:rbac:groups=private.azurerm.kubeform.com,resources=dnscnamerecords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=private.azurerm.kubeform.com,resources=dnscnamerecords/status,verbs=get;update;patch

func (r *DnsCnameRecordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dnscnamerecord", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DnsCnameRecord")
...
func (r *DnsCnameRecordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatev1alpha1.DnsCnameRecord{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatev1alpha1.DnsCnameRecord{}.APIVersion, privatev1alpha1.DnsCnameRecord{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatev1alpha1.DnsCnameRecord{}).
