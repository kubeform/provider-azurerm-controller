...
// +kubebuilder:rbac:groups=private.azurerm.kubeform.com,resources=dnsarecords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=private.azurerm.kubeform.com,resources=dnsarecords/status,verbs=get;update;patch

func (r *DnsARecordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dnsarecord", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DnsARecord")
...
func (r *DnsARecordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatev1alpha1.DnsARecord{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatev1alpha1.DnsARecord{}.APIVersion, privatev1alpha1.DnsARecord{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatev1alpha1.DnsARecord{}).
