...
// DnsPtrRecordReconciler reconciles a DnsPtrRecord object
type DnsPtrRecordReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=private.azurerm.kubeform.com,resources=dnsptrrecords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=private.azurerm.kubeform.com,resources=dnsptrrecords/status,verbs=get;update;patch

func (r *DnsPtrRecordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dnsptrrecord", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DnsPtrRecord")
...
func (r *DnsPtrRecordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatev1alpha1.DnsPtrRecord{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatev1alpha1.DnsPtrRecord{}.APIVersion, privatev1alpha1.DnsPtrRecord{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatev1alpha1.DnsPtrRecord{}).
