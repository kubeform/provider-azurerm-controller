...
// DnsZoneVirtualNetworkLinkReconciler reconciles a DnsZoneVirtualNetworkLink object
type DnsZoneVirtualNetworkLinkReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=private.azurerm.kubeform.com,resources=dnszonevirtualnetworklinks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=private.azurerm.kubeform.com,resources=dnszonevirtualnetworklinks/status,verbs=get;update;patch

func (r *DnsZoneVirtualNetworkLinkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dnszonevirtualnetworklink", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DnsZoneVirtualNetworkLink")
...
func (r *DnsZoneVirtualNetworkLinkReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatev1alpha1.DnsZoneVirtualNetworkLink{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatev1alpha1.DnsZoneVirtualNetworkLink{}.APIVersion, privatev1alpha1.DnsZoneVirtualNetworkLink{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatev1alpha1.DnsZoneVirtualNetworkLink{}).
