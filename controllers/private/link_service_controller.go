...
// LinkServiceReconciler reconciles a LinkService object
type LinkServiceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=private.azurerm.kubeform.com,resources=linkservices,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=private.azurerm.kubeform.com,resources=linkservices/status,verbs=get;update;patch

func (r *LinkServiceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("linkservice", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LinkService")
...
func (r *LinkServiceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatev1alpha1.LinkService{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatev1alpha1.LinkService{}.APIVersion, privatev1alpha1.LinkService{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatev1alpha1.LinkService{}).
