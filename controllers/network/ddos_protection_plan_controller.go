...
// DdosProtectionPlanReconciler reconciles a DdosProtectionPlan object
type DdosProtectionPlanReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=network.azurerm.kubeform.com,resources=ddosprotectionplans,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=network.azurerm.kubeform.com,resources=ddosprotectionplans/status,verbs=get;update;patch

func (r *DdosProtectionPlanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ddosprotectionplan", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DdosProtectionPlan")
...
func (r *DdosProtectionPlanReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkv1alpha1.DdosProtectionPlan{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkv1alpha1.DdosProtectionPlan{}.APIVersion, networkv1alpha1.DdosProtectionPlan{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.DdosProtectionPlan{}).
