...
// +kubebuilder:rbac:groups=network.azurerm.kubeform.com,resources=packetcaptures,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=network.azurerm.kubeform.com,resources=packetcaptures/status,verbs=get;update;patch

func (r *PacketCaptureReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("packetcapture", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PacketCapture")
...
func (r *PacketCaptureReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkv1alpha1.PacketCapture{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkv1alpha1.PacketCapture{}.APIVersion, networkv1alpha1.PacketCapture{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.PacketCapture{}).
