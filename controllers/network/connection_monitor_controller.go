...
// ConnectionMonitorReconciler reconciles a ConnectionMonitor object
type ConnectionMonitorReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=network.azurerm.kubeform.com,resources=connectionmonitors,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=network.azurerm.kubeform.com,resources=connectionmonitors/status,verbs=get;update;patch

func (r *ConnectionMonitorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("connectionmonitor", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectionMonitor")
...
func (r *ConnectionMonitorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkv1alpha1.ConnectionMonitor{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkv1alpha1.ConnectionMonitor{}.APIVersion, networkv1alpha1.ConnectionMonitor{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.ConnectionMonitor{}).
