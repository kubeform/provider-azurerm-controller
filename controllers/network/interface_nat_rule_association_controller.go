...
// InterfaceNATRuleAssociationReconciler reconciles a InterfaceNATRuleAssociation object
type InterfaceNATRuleAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=network.azurerm.kubeform.com,resources=interfacenatruleassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=network.azurerm.kubeform.com,resources=interfacenatruleassociations/status,verbs=get;update;patch

func (r *InterfaceNATRuleAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("interfacenatruleassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InterfaceNATRuleAssociation")
...
func (r *InterfaceNATRuleAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkv1alpha1.InterfaceNATRuleAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkv1alpha1.InterfaceNATRuleAssociation{}.APIVersion, networkv1alpha1.InterfaceNATRuleAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.InterfaceNATRuleAssociation{}).
