...
// SharedImageReconciler reconciles a SharedImage object
type SharedImageReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sharedimage.azurerm.kubeform.com,resources=sharedimages,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sharedimage.azurerm.kubeform.com,resources=sharedimages/status,verbs=get;update;patch

func (r *SharedImageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sharedimage", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SharedImage")
...
func (r *SharedImageReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sharedimagev1alpha1.SharedImage{}); err != nil {
			klog.Error(err, "unable to set up auditor", sharedimagev1alpha1.SharedImage{}.APIVersion, sharedimagev1alpha1.SharedImage{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sharedimagev1alpha1.SharedImage{}).
