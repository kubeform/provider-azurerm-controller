...
// GalleryReconciler reconciles a Gallery object
type GalleryReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sharedimage.azurerm.kubeform.com,resources=galleries,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sharedimage.azurerm.kubeform.com,resources=galleries/status,verbs=get;update;patch

func (r *GalleryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gallery", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Gallery")
...
func (r *GalleryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sharedimagev1alpha1.Gallery{}); err != nil {
			klog.Error(err, "unable to set up auditor", sharedimagev1alpha1.Gallery{}.APIVersion, sharedimagev1alpha1.Gallery{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sharedimagev1alpha1.Gallery{}).
