...
// AnalyzerEdgeModuleReconciler reconciles a AnalyzerEdgeModule object
type AnalyzerEdgeModuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=video.azurerm.kubeform.com,resources=analyzeredgemodules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=video.azurerm.kubeform.com,resources=analyzeredgemodules/status,verbs=get;update;patch

func (r *AnalyzerEdgeModuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyzeredgemodule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyzerEdgeModule")
...
func (r *AnalyzerEdgeModuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &videov1alpha1.AnalyzerEdgeModule{}); err != nil {
			klog.Error(err, "unable to set up auditor", videov1alpha1.AnalyzerEdgeModule{}.APIVersion, videov1alpha1.AnalyzerEdgeModule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&videov1alpha1.AnalyzerEdgeModule{}).
