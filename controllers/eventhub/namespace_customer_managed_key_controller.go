...
// NamespaceCustomerManagedKeyReconciler reconciles a NamespaceCustomerManagedKey object
type NamespaceCustomerManagedKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=eventhub.azurerm.kubeform.com,resources=namespacecustomermanagedkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=eventhub.azurerm.kubeform.com,resources=namespacecustomermanagedkeys/status,verbs=get;update;patch

func (r *NamespaceCustomerManagedKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("namespacecustomermanagedkey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NamespaceCustomerManagedKey")
...
func (r *NamespaceCustomerManagedKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eventhubv1alpha1.NamespaceCustomerManagedKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", eventhubv1alpha1.NamespaceCustomerManagedKey{}.APIVersion, eventhubv1alpha1.NamespaceCustomerManagedKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eventhubv1alpha1.NamespaceCustomerManagedKey{}).
