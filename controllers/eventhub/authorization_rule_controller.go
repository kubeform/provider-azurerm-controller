...
// AuthorizationRuleReconciler reconciles a AuthorizationRule object
type AuthorizationRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=eventhub.azurerm.kubeform.com,resources=authorizationrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=eventhub.azurerm.kubeform.com,resources=authorizationrules/status,verbs=get;update;patch

func (r *AuthorizationRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("authorizationrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AuthorizationRule")
...
func (r *AuthorizationRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eventhubv1alpha1.AuthorizationRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", eventhubv1alpha1.AuthorizationRule{}.APIVersion, eventhubv1alpha1.AuthorizationRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eventhubv1alpha1.AuthorizationRule{}).
...
func (r *AuthorizationRuleReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for AuthorizationRule")
			return nil
		}
		...
		resourceList := &eventhubv1alpha1.AuthorizationRuleList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AuthorizationRule")
			return nil
		}
