...
// NestedEndpointReconciler reconciles a NestedEndpoint object
type NestedEndpointReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=trafficmanager.azurerm.kubeform.com,resources=nestedendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=trafficmanager.azurerm.kubeform.com,resources=nestedendpoints/status,verbs=get;update;patch

func (r *NestedEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("nestedendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NestedEndpoint")
...
func (r *NestedEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &trafficmanagerv1alpha1.NestedEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", trafficmanagerv1alpha1.NestedEndpoint{}.APIVersion, trafficmanagerv1alpha1.NestedEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&trafficmanagerv1alpha1.NestedEndpoint{}).
