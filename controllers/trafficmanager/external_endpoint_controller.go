...
// ExternalEndpointReconciler reconciles a ExternalEndpoint object
type ExternalEndpointReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=trafficmanager.azurerm.kubeform.com,resources=externalendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=trafficmanager.azurerm.kubeform.com,resources=externalendpoints/status,verbs=get;update;patch

func (r *ExternalEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("externalendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ExternalEndpoint")
...
func (r *ExternalEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &trafficmanagerv1alpha1.ExternalEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", trafficmanagerv1alpha1.ExternalEndpoint{}.APIVersion, trafficmanagerv1alpha1.ExternalEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&trafficmanagerv1alpha1.ExternalEndpoint{}).
