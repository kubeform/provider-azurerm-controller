...
// AzureEndpointReconciler reconciles a AzureEndpoint object
type AzureEndpointReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=trafficmanager.azurerm.kubeform.com,resources=azureendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=trafficmanager.azurerm.kubeform.com,resources=azureendpoints/status,verbs=get;update;patch

func (r *AzureEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("azureendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AzureEndpoint")
...
func (r *AzureEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &trafficmanagerv1alpha1.AzureEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", trafficmanagerv1alpha1.AzureEndpoint{}.APIVersion, trafficmanagerv1alpha1.AzureEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&trafficmanagerv1alpha1.AzureEndpoint{}).
