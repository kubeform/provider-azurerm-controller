...
// ServiceNetworkACLReconciler reconciles a ServiceNetworkACL object
type ServiceNetworkACLReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=signalr.azurerm.kubeform.com,resources=servicenetworkacls,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=signalr.azurerm.kubeform.com,resources=servicenetworkacls/status,verbs=get;update;patch

func (r *ServiceNetworkACLReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicenetworkacl", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceNetworkACL")
...
func (r *ServiceNetworkACLReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &signalrv1alpha1.ServiceNetworkACL{}); err != nil {
			klog.Error(err, "unable to set up auditor", signalrv1alpha1.ServiceNetworkACL{}.APIVersion, signalrv1alpha1.ServiceNetworkACL{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&signalrv1alpha1.ServiceNetworkACL{}).
