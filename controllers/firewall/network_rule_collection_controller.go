...
// NetworkRuleCollectionReconciler reconciles a NetworkRuleCollection object
type NetworkRuleCollectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=firewall.azurerm.kubeform.com,resources=networkrulecollections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=firewall.azurerm.kubeform.com,resources=networkrulecollections/status,verbs=get;update;patch

func (r *NetworkRuleCollectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networkrulecollection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkRuleCollection")
...
func (r *NetworkRuleCollectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &firewallv1alpha1.NetworkRuleCollection{}); err != nil {
			klog.Error(err, "unable to set up auditor", firewallv1alpha1.NetworkRuleCollection{}.APIVersion, firewallv1alpha1.NetworkRuleCollection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&firewallv1alpha1.NetworkRuleCollection{}).
