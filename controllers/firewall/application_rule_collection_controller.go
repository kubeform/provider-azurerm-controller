...
// ApplicationRuleCollectionReconciler reconciles a ApplicationRuleCollection object
type ApplicationRuleCollectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=firewall.azurerm.kubeform.com,resources=applicationrulecollections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=firewall.azurerm.kubeform.com,resources=applicationrulecollections/status,verbs=get;update;patch

func (r *ApplicationRuleCollectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("applicationrulecollection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApplicationRuleCollection")
...
func (r *ApplicationRuleCollectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &firewallv1alpha1.ApplicationRuleCollection{}); err != nil {
			klog.Error(err, "unable to set up auditor", firewallv1alpha1.ApplicationRuleCollection{}.APIVersion, firewallv1alpha1.ApplicationRuleCollection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&firewallv1alpha1.ApplicationRuleCollection{}).
