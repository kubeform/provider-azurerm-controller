...
// NatRuleCollectionReconciler reconciles a NatRuleCollection object
type NatRuleCollectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=firewall.azurerm.kubeform.com,resources=natrulecollections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=firewall.azurerm.kubeform.com,resources=natrulecollections/status,verbs=get;update;patch

func (r *NatRuleCollectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("natrulecollection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NatRuleCollection")
...
func (r *NatRuleCollectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &firewallv1alpha1.NatRuleCollection{}); err != nil {
			klog.Error(err, "unable to set up auditor", firewallv1alpha1.NatRuleCollection{}.APIVersion, firewallv1alpha1.NatRuleCollection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&firewallv1alpha1.NatRuleCollection{}).
