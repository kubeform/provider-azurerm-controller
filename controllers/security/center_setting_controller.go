...
// CenterSettingReconciler reconciles a CenterSetting object
type CenterSettingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=security.azurerm.kubeform.com,resources=centersettings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=security.azurerm.kubeform.com,resources=centersettings/status,verbs=get;update;patch

func (r *CenterSettingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("centersetting", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CenterSetting")
...
func (r *CenterSettingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &securityv1alpha1.CenterSetting{}); err != nil {
			klog.Error(err, "unable to set up auditor", securityv1alpha1.CenterSetting{}.APIVersion, securityv1alpha1.CenterSetting{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&securityv1alpha1.CenterSetting{}).
