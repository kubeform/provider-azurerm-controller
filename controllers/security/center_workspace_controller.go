...
// CenterWorkspaceReconciler reconciles a CenterWorkspace object
type CenterWorkspaceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=security.azurerm.kubeform.com,resources=centerworkspaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=security.azurerm.kubeform.com,resources=centerworkspaces/status,verbs=get;update;patch

func (r *CenterWorkspaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("centerworkspace", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CenterWorkspace")
...
func (r *CenterWorkspaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &securityv1alpha1.CenterWorkspace{}); err != nil {
			klog.Error(err, "unable to set up auditor", securityv1alpha1.CenterWorkspace{}.APIVersion, securityv1alpha1.CenterWorkspace{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&securityv1alpha1.CenterWorkspace{}).
