...
// CenterAutomationReconciler reconciles a CenterAutomation object
type CenterAutomationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=security.azurerm.kubeform.com,resources=centerautomations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=security.azurerm.kubeform.com,resources=centerautomations/status,verbs=get;update;patch

func (r *CenterAutomationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("centerautomation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CenterAutomation")
...
func (r *CenterAutomationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &securityv1alpha1.CenterAutomation{}); err != nil {
			klog.Error(err, "unable to set up auditor", securityv1alpha1.CenterAutomation{}.APIVersion, securityv1alpha1.CenterAutomation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&securityv1alpha1.CenterAutomation{}).
...
func (r *CenterAutomationReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for CenterAutomation")
			return nil
		}
		...
		resourceList := &securityv1alpha1.CenterAutomationList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list CenterAutomation")
			return nil
		}
