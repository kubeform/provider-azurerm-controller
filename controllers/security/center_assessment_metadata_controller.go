...
// CenterAssessmentMetadataReconciler reconciles a CenterAssessmentMetadata object
type CenterAssessmentMetadataReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=security.azurerm.kubeform.com,resources=centerassessmentmetadata,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=security.azurerm.kubeform.com,resources=centerassessmentmetadata/status,verbs=get;update;patch

func (r *CenterAssessmentMetadataReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("centerassessmentmetadata", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CenterAssessmentMetadata")
...
func (r *CenterAssessmentMetadataReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &securityv1alpha1.CenterAssessmentMetadata{}); err != nil {
			klog.Error(err, "unable to set up auditor", securityv1alpha1.CenterAssessmentMetadata{}.APIVersion, securityv1alpha1.CenterAssessmentMetadata{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&securityv1alpha1.CenterAssessmentMetadata{}).
