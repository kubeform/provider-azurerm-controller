...
// +kubebuilder:rbac:groups=kubernetescluster.azurerm.kubeform.com,resources=kubernetesclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kubernetescluster.azurerm.kubeform.com,resources=kubernetesclusters/status,verbs=get;update;patch

func (r *KubernetesClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("kubernetescluster", req.NamespacedName)
...
func (r *KubernetesClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kubernetesclusterv1alpha1.KubernetesCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", kubernetesclusterv1alpha1.KubernetesCluster{}.APIVersion, kubernetesclusterv1alpha1.KubernetesCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kubernetesclusterv1alpha1.KubernetesCluster{}).
...
func (r *KubernetesClusterReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for KubernetesCluster")
			return nil
		}
		...
		resourceList := &kubernetesclusterv1alpha1.KubernetesClusterList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list KubernetesCluster")
			return nil
		}
