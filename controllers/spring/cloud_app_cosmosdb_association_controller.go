...
// CloudAppCosmosdbAssociationReconciler reconciles a CloudAppCosmosdbAssociation object
type CloudAppCosmosdbAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=spring.azurerm.kubeform.com,resources=cloudappcosmosdbassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=spring.azurerm.kubeform.com,resources=cloudappcosmosdbassociations/status,verbs=get;update;patch

func (r *CloudAppCosmosdbAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cloudappcosmosdbassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CloudAppCosmosdbAssociation")
...
func (r *CloudAppCosmosdbAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &springv1alpha1.CloudAppCosmosdbAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", springv1alpha1.CloudAppCosmosdbAssociation{}.APIVersion, springv1alpha1.CloudAppCosmosdbAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&springv1alpha1.CloudAppCosmosdbAssociation{}).
