...
// +kubebuilder:rbac:groups=spring.azurerm.kubeform.com,resources=cloudservices,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=spring.azurerm.kubeform.com,resources=cloudservices/status,verbs=get;update;patch

func (r *CloudServiceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cloudservice", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CloudService")
...
func (r *CloudServiceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &springv1alpha1.CloudService{}); err != nil {
			klog.Error(err, "unable to set up auditor", springv1alpha1.CloudService{}.APIVersion, springv1alpha1.CloudService{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&springv1alpha1.CloudService{}).
...
func (r *CloudServiceReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for CloudService")
			return nil
		}
		...
		resourceList := &springv1alpha1.CloudServiceList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list CloudService")
			return nil
		}
