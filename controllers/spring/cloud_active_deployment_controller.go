...
// CloudActiveDeploymentReconciler reconciles a CloudActiveDeployment object
type CloudActiveDeploymentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=spring.azurerm.kubeform.com,resources=cloudactivedeployments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=spring.azurerm.kubeform.com,resources=cloudactivedeployments/status,verbs=get;update;patch

func (r *CloudActiveDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cloudactivedeployment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CloudActiveDeployment")
...
func (r *CloudActiveDeploymentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &springv1alpha1.CloudActiveDeployment{}); err != nil {
			klog.Error(err, "unable to set up auditor", springv1alpha1.CloudActiveDeployment{}.APIVersion, springv1alpha1.CloudActiveDeployment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&springv1alpha1.CloudActiveDeployment{}).
