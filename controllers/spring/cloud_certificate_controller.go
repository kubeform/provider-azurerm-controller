...
// CloudCertificateReconciler reconciles a CloudCertificate object
type CloudCertificateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=spring.azurerm.kubeform.com,resources=cloudcertificates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=spring.azurerm.kubeform.com,resources=cloudcertificates/status,verbs=get;update;patch

func (r *CloudCertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cloudcertificate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CloudCertificate")
...
func (r *CloudCertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &springv1alpha1.CloudCertificate{}); err != nil {
			klog.Error(err, "unable to set up auditor", springv1alpha1.CloudCertificate{}.APIVersion, springv1alpha1.CloudCertificate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&springv1alpha1.CloudCertificate{}).
