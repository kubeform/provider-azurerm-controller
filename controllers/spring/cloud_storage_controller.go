...
// +kubebuilder:rbac:groups=spring.azurerm.kubeform.com,resources=cloudstorages,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=spring.azurerm.kubeform.com,resources=cloudstorages/status,verbs=get;update;patch

func (r *CloudStorageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cloudstorage", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CloudStorage")
...
func (r *CloudStorageReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &springv1alpha1.CloudStorage{}); err != nil {
			klog.Error(err, "unable to set up auditor", springv1alpha1.CloudStorage{}.APIVersion, springv1alpha1.CloudStorage{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&springv1alpha1.CloudStorage{}).
