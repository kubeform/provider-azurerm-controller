...
// CloudAppMysqlAssociationReconciler reconciles a CloudAppMysqlAssociation object
type CloudAppMysqlAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=spring.azurerm.kubeform.com,resources=cloudappmysqlassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=spring.azurerm.kubeform.com,resources=cloudappmysqlassociations/status,verbs=get;update;patch

func (r *CloudAppMysqlAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cloudappmysqlassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CloudAppMysqlAssociation")
...
func (r *CloudAppMysqlAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &springv1alpha1.CloudAppMysqlAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", springv1alpha1.CloudAppMysqlAssociation{}.APIVersion, springv1alpha1.CloudAppMysqlAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&springv1alpha1.CloudAppMysqlAssociation{}).
...
func (r *CloudAppMysqlAssociationReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for CloudAppMysqlAssociation")
			return nil
		}
		...
		resourceList := &springv1alpha1.CloudAppMysqlAssociationList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list CloudAppMysqlAssociation")
			return nil
		}
