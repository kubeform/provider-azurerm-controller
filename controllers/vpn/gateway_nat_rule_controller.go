...
// GatewayNATRuleReconciler reconciles a GatewayNATRule object
type GatewayNATRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpn.azurerm.kubeform.com,resources=gatewaynatrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpn.azurerm.kubeform.com,resources=gatewaynatrules/status,verbs=get;update;patch

func (r *GatewayNATRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gatewaynatrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GatewayNATRule")
...
func (r *GatewayNATRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpnv1alpha1.GatewayNATRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpnv1alpha1.GatewayNATRule{}.APIVersion, vpnv1alpha1.GatewayNATRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpnv1alpha1.GatewayNATRule{}).
