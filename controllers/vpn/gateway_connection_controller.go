...
// GatewayConnectionReconciler reconciles a GatewayConnection object
type GatewayConnectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpn.azurerm.kubeform.com,resources=gatewayconnections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpn.azurerm.kubeform.com,resources=gatewayconnections/status,verbs=get;update;patch

func (r *GatewayConnectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gatewayconnection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GatewayConnection")
...
func (r *GatewayConnectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpnv1alpha1.GatewayConnection{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpnv1alpha1.GatewayConnection{}.APIVersion, vpnv1alpha1.GatewayConnection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpnv1alpha1.GatewayConnection{}).
