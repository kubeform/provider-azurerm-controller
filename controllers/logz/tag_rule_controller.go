...
// TagRuleReconciler reconciles a TagRule object
type TagRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logz.azurerm.kubeform.com,resources=tagrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logz.azurerm.kubeform.com,resources=tagrules/status,verbs=get;update;patch

func (r *TagRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tagrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TagRule")
...
func (r *TagRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logzv1alpha1.TagRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", logzv1alpha1.TagRule{}.APIVersion, logzv1alpha1.TagRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logzv1alpha1.TagRule{}).
