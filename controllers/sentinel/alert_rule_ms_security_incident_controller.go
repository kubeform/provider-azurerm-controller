...
// AlertRuleMsSecurityIncidentReconciler reconciles a AlertRuleMsSecurityIncident object
type AlertRuleMsSecurityIncidentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sentinel.azurerm.kubeform.com,resources=alertrulemssecurityincidents,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sentinel.azurerm.kubeform.com,resources=alertrulemssecurityincidents/status,verbs=get;update;patch

func (r *AlertRuleMsSecurityIncidentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alertrulemssecurityincident", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AlertRuleMsSecurityIncident")
...
func (r *AlertRuleMsSecurityIncidentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sentinelv1alpha1.AlertRuleMsSecurityIncident{}); err != nil {
			klog.Error(err, "unable to set up auditor", sentinelv1alpha1.AlertRuleMsSecurityIncident{}.APIVersion, sentinelv1alpha1.AlertRuleMsSecurityIncident{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sentinelv1alpha1.AlertRuleMsSecurityIncident{}).
