...
// WatchlistReconciler reconciles a Watchlist object
type WatchlistReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sentinel.azurerm.kubeform.com,resources=watchlists,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sentinel.azurerm.kubeform.com,resources=watchlists/status,verbs=get;update;patch

func (r *WatchlistReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("watchlist", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Watchlist")
...
func (r *WatchlistReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sentinelv1alpha1.Watchlist{}); err != nil {
			klog.Error(err, "unable to set up auditor", sentinelv1alpha1.Watchlist{}.APIVersion, sentinelv1alpha1.Watchlist{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sentinelv1alpha1.Watchlist{}).
