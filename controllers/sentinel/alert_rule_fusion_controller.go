...
// AlertRuleFusionReconciler reconciles a AlertRuleFusion object
type AlertRuleFusionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sentinel.azurerm.kubeform.com,resources=alertrulefusions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sentinel.azurerm.kubeform.com,resources=alertrulefusions/status,verbs=get;update;patch

func (r *AlertRuleFusionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alertrulefusion", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AlertRuleFusion")
...
func (r *AlertRuleFusionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sentinelv1alpha1.AlertRuleFusion{}); err != nil {
			klog.Error(err, "unable to set up auditor", sentinelv1alpha1.AlertRuleFusion{}.APIVersion, sentinelv1alpha1.AlertRuleFusion{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sentinelv1alpha1.AlertRuleFusion{}).
