...
// DataConnectorAwsCloudTrailReconciler reconciles a DataConnectorAwsCloudTrail object
type DataConnectorAwsCloudTrailReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sentinel.azurerm.kubeform.com,resources=dataconnectorawscloudtrails,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sentinel.azurerm.kubeform.com,resources=dataconnectorawscloudtrails/status,verbs=get;update;patch

func (r *DataConnectorAwsCloudTrailReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dataconnectorawscloudtrail", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DataConnectorAwsCloudTrail")
...
func (r *DataConnectorAwsCloudTrailReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sentinelv1alpha1.DataConnectorAwsCloudTrail{}); err != nil {
			klog.Error(err, "unable to set up auditor", sentinelv1alpha1.DataConnectorAwsCloudTrail{}.APIVersion, sentinelv1alpha1.DataConnectorAwsCloudTrail{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sentinelv1alpha1.DataConnectorAwsCloudTrail{}).
