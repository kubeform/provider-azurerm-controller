...
// AlertRuleScheduledReconciler reconciles a AlertRuleScheduled object
type AlertRuleScheduledReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sentinel.azurerm.kubeform.com,resources=alertrulescheduleds,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sentinel.azurerm.kubeform.com,resources=alertrulescheduleds/status,verbs=get;update;patch

func (r *AlertRuleScheduledReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alertrulescheduled", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AlertRuleScheduled")
...
func (r *AlertRuleScheduledReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sentinelv1alpha1.AlertRuleScheduled{}); err != nil {
			klog.Error(err, "unable to set up auditor", sentinelv1alpha1.AlertRuleScheduled{}.APIVersion, sentinelv1alpha1.AlertRuleScheduled{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sentinelv1alpha1.AlertRuleScheduled{}).
