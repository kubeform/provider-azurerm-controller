...
// +kubebuilder:rbac:groups=marketplace.azurerm.kubeform.com,resources=agreements,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=marketplace.azurerm.kubeform.com,resources=agreements/status,verbs=get;update;patch

func (r *AgreementReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("agreement", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Agreement")
...
func (r *AgreementReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &marketplacev1alpha1.Agreement{}); err != nil {
			klog.Error(err, "unable to set up auditor", marketplacev1alpha1.Agreement{}.APIVersion, marketplacev1alpha1.Agreement{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&marketplacev1alpha1.Agreement{}).
