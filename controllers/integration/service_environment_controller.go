...
// +kubebuilder:rbac:groups=integration.azurerm.kubeform.com,resources=serviceenvironments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=integration.azurerm.kubeform.com,resources=serviceenvironments/status,verbs=get;update;patch

func (r *ServiceEnvironmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serviceenvironment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceEnvironment")
...
func (r *ServiceEnvironmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &integrationv1alpha1.ServiceEnvironment{}); err != nil {
			klog.Error(err, "unable to set up auditor", integrationv1alpha1.ServiceEnvironment{}.APIVersion, integrationv1alpha1.ServiceEnvironment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&integrationv1alpha1.ServiceEnvironment{}).
