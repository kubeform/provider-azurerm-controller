...
// TwinsEndpointEventgridReconciler reconciles a TwinsEndpointEventgrid object
type TwinsEndpointEventgridReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=digital.azurerm.kubeform.com,resources=twinsendpointeventgrids,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=digital.azurerm.kubeform.com,resources=twinsendpointeventgrids/status,verbs=get;update;patch

func (r *TwinsEndpointEventgridReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("twinsendpointeventgrid", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TwinsEndpointEventgrid")
...
func (r *TwinsEndpointEventgridReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &digitalv1alpha1.TwinsEndpointEventgrid{}); err != nil {
			klog.Error(err, "unable to set up auditor", digitalv1alpha1.TwinsEndpointEventgrid{}.APIVersion, digitalv1alpha1.TwinsEndpointEventgrid{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&digitalv1alpha1.TwinsEndpointEventgrid{}).
