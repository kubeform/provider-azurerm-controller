...
// TwinsEndpointServicebusReconciler reconciles a TwinsEndpointServicebus object
type TwinsEndpointServicebusReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=digital.azurerm.kubeform.com,resources=twinsendpointservicebus/status,verbs=get;update;patch
...
func (r *TwinsEndpointServicebusReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TwinsEndpointServicebus")
...
func (r *TwinsEndpointServicebusReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
...
func (r *TwinsEndpointServicebusReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for TwinsEndpointServicebus")
			return nil
		}
		...
		resourceList := &digitalv1alpha1.TwinsEndpointServicebusList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list TwinsEndpointServicebus")
			return nil
		}
