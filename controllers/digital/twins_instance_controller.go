...
// TwinsInstanceReconciler reconciles a TwinsInstance object
type TwinsInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=digital.azurerm.kubeform.com,resources=twinsinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=digital.azurerm.kubeform.com,resources=twinsinstances/status,verbs=get;update;patch

func (r *TwinsInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("twinsinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TwinsInstance")
...
func (r *TwinsInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &digitalv1alpha1.TwinsInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", digitalv1alpha1.TwinsInstance{}.APIVersion, digitalv1alpha1.TwinsInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&digitalv1alpha1.TwinsInstance{}).
