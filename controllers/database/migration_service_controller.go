...
// MigrationServiceReconciler reconciles a MigrationService object
type MigrationServiceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=database.azurerm.kubeform.com,resources=migrationservices,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.azurerm.kubeform.com,resources=migrationservices/status,verbs=get;update;patch

func (r *MigrationServiceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("migrationservice", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MigrationService")
...
func (r *MigrationServiceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.MigrationService{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.MigrationService{}.APIVersion, databasev1alpha1.MigrationService{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.MigrationService{}).
