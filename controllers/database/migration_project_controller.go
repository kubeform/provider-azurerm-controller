...
// MigrationProjectReconciler reconciles a MigrationProject object
type MigrationProjectReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=database.azurerm.kubeform.com,resources=migrationprojects,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.azurerm.kubeform.com,resources=migrationprojects/status,verbs=get;update;patch

func (r *MigrationProjectReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("migrationproject", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MigrationProject")
...
func (r *MigrationProjectReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.MigrationProject{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.MigrationProject{}.APIVersion, databasev1alpha1.MigrationProject{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.MigrationProject{}).
