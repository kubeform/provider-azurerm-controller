...
// DirectoryDomainServiceReconciler reconciles a DirectoryDomainService object
type DirectoryDomainServiceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=active.azurerm.kubeform.com,resources=directorydomainservices,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=active.azurerm.kubeform.com,resources=directorydomainservices/status,verbs=get;update;patch

func (r *DirectoryDomainServiceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("directorydomainservice", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DirectoryDomainService")
...
func (r *DirectoryDomainServiceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &activev1alpha1.DirectoryDomainService{}); err != nil {
			klog.Error(err, "unable to set up auditor", activev1alpha1.DirectoryDomainService{}.APIVersion, activev1alpha1.DirectoryDomainService{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&activev1alpha1.DirectoryDomainService{}).
...
func (r *DirectoryDomainServiceReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for DirectoryDomainService")
			return nil
		}
		...
		resourceList := &activev1alpha1.DirectoryDomainServiceList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list DirectoryDomainService")
			return nil
		}
