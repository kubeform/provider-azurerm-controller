...
// LogProfileReconciler reconciles a LogProfile object
type LogProfileReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=monitor.azurerm.kubeform.com,resources=logprofiles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=monitor.azurerm.kubeform.com,resources=logprofiles/status,verbs=get;update;patch

func (r *LogProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("logprofile", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LogProfile")
...
func (r *LogProfileReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &monitorv1alpha1.LogProfile{}); err != nil {
			klog.Error(err, "unable to set up auditor", monitorv1alpha1.LogProfile{}.APIVersion, monitorv1alpha1.LogProfile{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&monitorv1alpha1.LogProfile{}).
