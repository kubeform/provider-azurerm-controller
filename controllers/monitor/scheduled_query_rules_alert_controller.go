...
// ScheduledQueryRulesAlertReconciler reconciles a ScheduledQueryRulesAlert object
type ScheduledQueryRulesAlertReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=monitor.azurerm.kubeform.com,resources=scheduledqueryrulesalerts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=monitor.azurerm.kubeform.com,resources=scheduledqueryrulesalerts/status,verbs=get;update;patch

func (r *ScheduledQueryRulesAlertReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("scheduledqueryrulesalert", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ScheduledQueryRulesAlert")
...
func (r *ScheduledQueryRulesAlertReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &monitorv1alpha1.ScheduledQueryRulesAlert{}); err != nil {
			klog.Error(err, "unable to set up auditor", monitorv1alpha1.ScheduledQueryRulesAlert{}.APIVersion, monitorv1alpha1.ScheduledQueryRulesAlert{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&monitorv1alpha1.ScheduledQueryRulesAlert{}).
