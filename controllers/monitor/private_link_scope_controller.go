...
// PrivateLinkScopeReconciler reconciles a PrivateLinkScope object
type PrivateLinkScopeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=monitor.azurerm.kubeform.com,resources=privatelinkscopes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=monitor.azurerm.kubeform.com,resources=privatelinkscopes/status,verbs=get;update;patch

func (r *PrivateLinkScopeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("privatelinkscope", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PrivateLinkScope")
...
func (r *PrivateLinkScopeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &monitorv1alpha1.PrivateLinkScope{}); err != nil {
			klog.Error(err, "unable to set up auditor", monitorv1alpha1.PrivateLinkScope{}.APIVersion, monitorv1alpha1.PrivateLinkScope{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&monitorv1alpha1.PrivateLinkScope{}).
