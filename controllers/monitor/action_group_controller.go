...
// ActionGroupReconciler reconciles a ActionGroup object
...
// +kubebuilder:rbac:groups=monitor.azurerm.kubeform.com,resources=actiongroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=monitor.azurerm.kubeform.com,resources=actiongroups/status,verbs=get;update;patch

func (r *ActionGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("actiongroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ActionGroup")
...
func (r *ActionGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &monitorv1alpha1.ActionGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", monitorv1alpha1.ActionGroup{}.APIVersion, monitorv1alpha1.ActionGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&monitorv1alpha1.ActionGroup{}).
