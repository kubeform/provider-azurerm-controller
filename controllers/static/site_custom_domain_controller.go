...
// SiteCustomDomainReconciler reconciles a SiteCustomDomain object
type SiteCustomDomainReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=static.azurerm.kubeform.com,resources=sitecustomdomains,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=static.azurerm.kubeform.com,resources=sitecustomdomains/status,verbs=get;update;patch

func (r *SiteCustomDomainReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sitecustomdomain", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SiteCustomDomain")
...
func (r *SiteCustomDomainReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &staticv1alpha1.SiteCustomDomain{}); err != nil {
			klog.Error(err, "unable to set up auditor", staticv1alpha1.SiteCustomDomain{}.APIVersion, staticv1alpha1.SiteCustomDomain{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&staticv1alpha1.SiteCustomDomain{}).
...
func (r *SiteCustomDomainReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for SiteCustomDomain")
			return nil
		}
		...
		resourceList := &staticv1alpha1.SiteCustomDomainList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SiteCustomDomain")
			return nil
		}
