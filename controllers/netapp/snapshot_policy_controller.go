...
// +kubebuilder:rbac:groups=netapp.azurerm.kubeform.com,resources=snapshotpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=netapp.azurerm.kubeform.com,resources=snapshotpolicies/status,verbs=get;update;patch

func (r *SnapshotPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("snapshotpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SnapshotPolicy")
...
func (r *SnapshotPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &netappv1alpha1.SnapshotPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", netappv1alpha1.SnapshotPolicy{}.APIVersion, netappv1alpha1.SnapshotPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&netappv1alpha1.SnapshotPolicy{}).
