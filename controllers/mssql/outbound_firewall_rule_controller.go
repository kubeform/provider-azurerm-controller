...
// OutboundFirewallRuleReconciler reconciles a OutboundFirewallRule object
type OutboundFirewallRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=mssql.azurerm.kubeform.com,resources=outboundfirewallrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mssql.azurerm.kubeform.com,resources=outboundfirewallrules/status,verbs=get;update;patch

func (r *OutboundFirewallRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("outboundfirewallrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OutboundFirewallRule")
...
func (r *OutboundFirewallRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mssqlv1alpha1.OutboundFirewallRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", mssqlv1alpha1.OutboundFirewallRule{}.APIVersion, mssqlv1alpha1.OutboundFirewallRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mssqlv1alpha1.OutboundFirewallRule{}).
