...
// ServerExtendedAuditingPolicyReconciler reconciles a ServerExtendedAuditingPolicy object
type ServerExtendedAuditingPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=mssql.azurerm.kubeform.com,resources=serverextendedauditingpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mssql.azurerm.kubeform.com,resources=serverextendedauditingpolicies/status,verbs=get;update;patch

func (r *ServerExtendedAuditingPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serverextendedauditingpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServerExtendedAuditingPolicy")
...
func (r *ServerExtendedAuditingPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mssqlv1alpha1.ServerExtendedAuditingPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", mssqlv1alpha1.ServerExtendedAuditingPolicy{}.APIVersion, mssqlv1alpha1.ServerExtendedAuditingPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mssqlv1alpha1.ServerExtendedAuditingPolicy{}).
...
func (r *ServerExtendedAuditingPolicyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ServerExtendedAuditingPolicy")
			return nil
		}
		...
		resourceList := &mssqlv1alpha1.ServerExtendedAuditingPolicyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ServerExtendedAuditingPolicy")
			return nil
		}
