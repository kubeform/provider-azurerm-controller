...
// +kubebuilder:rbac:groups=mssql.azurerm.kubeform.com,resources=failovergroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mssql.azurerm.kubeform.com,resources=failovergroups/status,verbs=get;update;patch
...
func (r *FailoverGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FailoverGroup")
...
func (r *FailoverGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mssqlv1alpha1.FailoverGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", mssqlv1alpha1.FailoverGroup{}.APIVersion, mssqlv1alpha1.FailoverGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mssqlv1alpha1.FailoverGroup{}).
