...
// ServerTransparentDataEncryptionReconciler reconciles a ServerTransparentDataEncryption object
type ServerTransparentDataEncryptionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=mssql.azurerm.kubeform.com,resources=servertransparentdataencryptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mssql.azurerm.kubeform.com,resources=servertransparentdataencryptions/status,verbs=get;update;patch

func (r *ServerTransparentDataEncryptionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servertransparentdataencryption", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServerTransparentDataEncryption")
...
func (r *ServerTransparentDataEncryptionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mssqlv1alpha1.ServerTransparentDataEncryption{}); err != nil {
			klog.Error(err, "unable to set up auditor", mssqlv1alpha1.ServerTransparentDataEncryption{}.APIVersion, mssqlv1alpha1.ServerTransparentDataEncryption{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mssqlv1alpha1.ServerTransparentDataEncryption{}).
