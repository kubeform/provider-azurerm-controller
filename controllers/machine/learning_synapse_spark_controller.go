...
// LearningSynapseSparkReconciler reconciles a LearningSynapseSpark object
type LearningSynapseSparkReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=machine.azurerm.kubeform.com,resources=learningsynapsesparks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=machine.azurerm.kubeform.com,resources=learningsynapsesparks/status,verbs=get;update;patch

func (r *LearningSynapseSparkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("learningsynapsespark", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LearningSynapseSpark")
...
func (r *LearningSynapseSparkReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &machinev1alpha1.LearningSynapseSpark{}); err != nil {
			klog.Error(err, "unable to set up auditor", machinev1alpha1.LearningSynapseSpark{}.APIVersion, machinev1alpha1.LearningSynapseSpark{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&machinev1alpha1.LearningSynapseSpark{}).
