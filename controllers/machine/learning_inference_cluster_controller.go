...
// LearningInferenceClusterReconciler reconciles a LearningInferenceCluster object
type LearningInferenceClusterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=machine.azurerm.kubeform.com,resources=learninginferenceclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=machine.azurerm.kubeform.com,resources=learninginferenceclusters/status,verbs=get;update;patch

func (r *LearningInferenceClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("learninginferencecluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LearningInferenceCluster")
...
func (r *LearningInferenceClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &machinev1alpha1.LearningInferenceCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", machinev1alpha1.LearningInferenceCluster{}.APIVersion, machinev1alpha1.LearningInferenceCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&machinev1alpha1.LearningInferenceCluster{}).
