...
// LearningComputeInstanceReconciler reconciles a LearningComputeInstance object
type LearningComputeInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=machine.azurerm.kubeform.com,resources=learningcomputeinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=machine.azurerm.kubeform.com,resources=learningcomputeinstances/status,verbs=get;update;patch

func (r *LearningComputeInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("learningcomputeinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LearningComputeInstance")
...
func (r *LearningComputeInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &machinev1alpha1.LearningComputeInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", machinev1alpha1.LearningComputeInstance{}.APIVersion, machinev1alpha1.LearningComputeInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&machinev1alpha1.LearningComputeInstance{}).
