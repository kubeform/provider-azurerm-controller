...
// LearningComputeClusterReconciler reconciles a LearningComputeCluster object
type LearningComputeClusterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=machine.azurerm.kubeform.com,resources=learningcomputeclusters/status,verbs=get;update;patch
...
func (r *LearningComputeClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LearningComputeCluster")
...
func (r *LearningComputeClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
