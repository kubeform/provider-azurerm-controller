...
// +kubebuilder:rbac:groups=load.azurerm.kubeform.com,resources=loadtests,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=load.azurerm.kubeform.com,resources=loadtests/status,verbs=get;update;patch
...
func (r *LoadtestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Loadtest")
...
func (r *LoadtestReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadv1alpha1.Loadtest{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadv1alpha1.Loadtest{}.APIVersion, loadv1alpha1.Loadtest{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadv1alpha1.Loadtest{}).
