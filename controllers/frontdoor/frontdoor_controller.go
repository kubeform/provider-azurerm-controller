...
// FrontdoorReconciler reconciles a Frontdoor object
type FrontdoorReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=frontdoor.azurerm.kubeform.com,resources=frontdoors,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=frontdoor.azurerm.kubeform.com,resources=frontdoors/status,verbs=get;update;patch

func (r *FrontdoorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("frontdoor", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Frontdoor")
...
func (r *FrontdoorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &frontdoorv1alpha1.Frontdoor{}); err != nil {
			klog.Error(err, "unable to set up auditor", frontdoorv1alpha1.Frontdoor{}.APIVersion, frontdoorv1alpha1.Frontdoor{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&frontdoorv1alpha1.Frontdoor{}).
