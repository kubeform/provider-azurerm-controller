...
// CustomHTTPSConfigurationReconciler reconciles a CustomHTTPSConfiguration object
type CustomHTTPSConfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=frontdoor.azurerm.kubeform.com,resources=customhttpsconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=frontdoor.azurerm.kubeform.com,resources=customhttpsconfigurations/status,verbs=get;update;patch

func (r *CustomHTTPSConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("customhttpsconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CustomHTTPSConfiguration")
...
func (r *CustomHTTPSConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &frontdoorv1alpha1.CustomHTTPSConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", frontdoorv1alpha1.CustomHTTPSConfiguration{}.APIVersion, frontdoorv1alpha1.CustomHTTPSConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&frontdoorv1alpha1.CustomHTTPSConfiguration{}).
