...
// ManagementExportResourceGroupReconciler reconciles a ManagementExportResourceGroup object
type ManagementExportResourceGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cost.azurerm.kubeform.com,resources=managementexportresourcegroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cost.azurerm.kubeform.com,resources=managementexportresourcegroups/status,verbs=get;update;patch

func (r *ManagementExportResourceGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managementexportresourcegroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagementExportResourceGroup")
...
func (r *ManagementExportResourceGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &costv1alpha1.ManagementExportResourceGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", costv1alpha1.ManagementExportResourceGroup{}.APIVersion, costv1alpha1.ManagementExportResourceGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&costv1alpha1.ManagementExportResourceGroup{}).
