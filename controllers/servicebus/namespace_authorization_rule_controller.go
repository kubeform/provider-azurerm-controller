...
// NamespaceAuthorizationRuleReconciler reconciles a NamespaceAuthorizationRule object
type NamespaceAuthorizationRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=servicebus.azurerm.kubeform.com,resources=namespaceauthorizationrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servicebus.azurerm.kubeform.com,resources=namespaceauthorizationrules/status,verbs=get;update;patch

func (r *NamespaceAuthorizationRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("namespaceauthorizationrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NamespaceAuthorizationRule")
...
func (r *NamespaceAuthorizationRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicebusv1alpha1.NamespaceAuthorizationRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicebusv1alpha1.NamespaceAuthorizationRule{}.APIVersion, servicebusv1alpha1.NamespaceAuthorizationRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicebusv1alpha1.NamespaceAuthorizationRule{}).
...
func (r *NamespaceAuthorizationRuleReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for NamespaceAuthorizationRule")
			return nil
		}
		...
		resourceList := &servicebusv1alpha1.NamespaceAuthorizationRuleList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list NamespaceAuthorizationRule")
			return nil
		}
