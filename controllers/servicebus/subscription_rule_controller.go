...
// SubscriptionRuleReconciler reconciles a SubscriptionRule object
type SubscriptionRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=servicebus.azurerm.kubeform.com,resources=subscriptionrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servicebus.azurerm.kubeform.com,resources=subscriptionrules/status,verbs=get;update;patch

func (r *SubscriptionRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("subscriptionrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SubscriptionRule")
...
func (r *SubscriptionRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicebusv1alpha1.SubscriptionRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicebusv1alpha1.SubscriptionRule{}.APIVersion, servicebusv1alpha1.SubscriptionRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicebusv1alpha1.SubscriptionRule{}).
