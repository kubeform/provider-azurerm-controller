...
// NamespaceNetworkRuleSetReconciler reconciles a NamespaceNetworkRuleSet object
type NamespaceNetworkRuleSetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=servicebus.azurerm.kubeform.com,resources=namespacenetworkrulesets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servicebus.azurerm.kubeform.com,resources=namespacenetworkrulesets/status,verbs=get;update;patch

func (r *NamespaceNetworkRuleSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("namespacenetworkruleset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NamespaceNetworkRuleSet")
...
func (r *NamespaceNetworkRuleSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicebusv1alpha1.NamespaceNetworkRuleSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicebusv1alpha1.NamespaceNetworkRuleSet{}.APIVersion, servicebusv1alpha1.NamespaceNetworkRuleSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicebusv1alpha1.NamespaceNetworkRuleSet{}).
