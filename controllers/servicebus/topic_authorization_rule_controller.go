...
// TopicAuthorizationRuleReconciler reconciles a TopicAuthorizationRule object
type TopicAuthorizationRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=servicebus.azurerm.kubeform.com,resources=topicauthorizationrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servicebus.azurerm.kubeform.com,resources=topicauthorizationrules/status,verbs=get;update;patch

func (r *TopicAuthorizationRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("topicauthorizationrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TopicAuthorizationRule")
...
func (r *TopicAuthorizationRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicebusv1alpha1.TopicAuthorizationRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicebusv1alpha1.TopicAuthorizationRule{}.APIVersion, servicebusv1alpha1.TopicAuthorizationRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicebusv1alpha1.TopicAuthorizationRule{}).
...
func (r *TopicAuthorizationRuleReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for TopicAuthorizationRule")
			return nil
		}
		...
		resourceList := &servicebusv1alpha1.TopicAuthorizationRuleList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list TopicAuthorizationRule")
			return nil
		}
