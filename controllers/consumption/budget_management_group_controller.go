...
// BudgetManagementGroupReconciler reconciles a BudgetManagementGroup object
type BudgetManagementGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=consumption.azurerm.kubeform.com,resources=budgetmanagementgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=consumption.azurerm.kubeform.com,resources=budgetmanagementgroups/status,verbs=get;update;patch

func (r *BudgetManagementGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("budgetmanagementgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BudgetManagementGroup")
...
func (r *BudgetManagementGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &consumptionv1alpha1.BudgetManagementGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", consumptionv1alpha1.BudgetManagementGroup{}.APIVersion, consumptionv1alpha1.BudgetManagementGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&consumptionv1alpha1.BudgetManagementGroup{}).
