...
// BudgetSubscriptionReconciler reconciles a BudgetSubscription object
type BudgetSubscriptionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=consumption.azurerm.kubeform.com,resources=budgetsubscriptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=consumption.azurerm.kubeform.com,resources=budgetsubscriptions/status,verbs=get;update;patch

func (r *BudgetSubscriptionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("budgetsubscription", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BudgetSubscription")
...
func (r *BudgetSubscriptionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &consumptionv1alpha1.BudgetSubscription{}); err != nil {
			klog.Error(err, "unable to set up auditor", consumptionv1alpha1.BudgetSubscription{}.APIVersion, consumptionv1alpha1.BudgetSubscription{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&consumptionv1alpha1.BudgetSubscription{}).
