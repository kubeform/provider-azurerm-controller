...
// HybridConnectionReconciler reconciles a HybridConnection object
type HybridConnectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=relay.azurerm.kubeform.com,resources=hybridconnections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=relay.azurerm.kubeform.com,resources=hybridconnections/status,verbs=get;update;patch

func (r *HybridConnectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hybridconnection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HybridConnection")
...
func (r *HybridConnectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &relayv1alpha1.HybridConnection{}); err != nil {
			klog.Error(err, "unable to set up auditor", relayv1alpha1.HybridConnection{}.APIVersion, relayv1alpha1.HybridConnection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&relayv1alpha1.HybridConnection{}).
