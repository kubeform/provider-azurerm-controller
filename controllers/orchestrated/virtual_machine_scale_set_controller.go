...
// VirtualMachineScaleSetReconciler reconciles a VirtualMachineScaleSet object
type VirtualMachineScaleSetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=orchestrated.azurerm.kubeform.com,resources=virtualmachinescalesets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=orchestrated.azurerm.kubeform.com,resources=virtualmachinescalesets/status,verbs=get;update;patch

func (r *VirtualMachineScaleSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("virtualmachinescaleset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VirtualMachineScaleSet")
...
func (r *VirtualMachineScaleSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &orchestratedv1alpha1.VirtualMachineScaleSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", orchestratedv1alpha1.VirtualMachineScaleSet{}.APIVersion, orchestratedv1alpha1.VirtualMachineScaleSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&orchestratedv1alpha1.VirtualMachineScaleSet{}).
...
func (r *VirtualMachineScaleSetReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for VirtualMachineScaleSet")
			return nil
		}
		...
		resourceList := &orchestratedv1alpha1.VirtualMachineScaleSetList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list VirtualMachineScaleSet")
			return nil
		}
