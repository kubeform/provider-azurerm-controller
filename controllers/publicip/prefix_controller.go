...
// +kubebuilder:rbac:groups=publicip.azurerm.kubeform.com,resources=prefixes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=publicip.azurerm.kubeform.com,resources=prefixes/status,verbs=get;update;patch

func (r *PrefixReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("prefix", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Prefix")
...
func (r *PrefixReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &publicipv1alpha1.Prefix{}); err != nil {
			klog.Error(err, "unable to set up auditor", publicipv1alpha1.Prefix{}.APIVersion, publicipv1alpha1.Prefix{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&publicipv1alpha1.Prefix{}).
