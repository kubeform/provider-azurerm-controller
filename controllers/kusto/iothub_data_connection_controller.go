...
// IothubDataConnectionReconciler reconciles a IothubDataConnection object
type IothubDataConnectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kusto.azurerm.kubeform.com,resources=iothubdataconnections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kusto.azurerm.kubeform.com,resources=iothubdataconnections/status,verbs=get;update;patch

func (r *IothubDataConnectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("iothubdataconnection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IothubDataConnection")
...
func (r *IothubDataConnectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kustov1alpha1.IothubDataConnection{}); err != nil {
			klog.Error(err, "unable to set up auditor", kustov1alpha1.IothubDataConnection{}.APIVersion, kustov1alpha1.IothubDataConnection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kustov1alpha1.IothubDataConnection{}).
