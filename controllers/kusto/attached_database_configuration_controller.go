...
// AttachedDatabaseConfigurationReconciler reconciles a AttachedDatabaseConfiguration object
type AttachedDatabaseConfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kusto.azurerm.kubeform.com,resources=attacheddatabaseconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kusto.azurerm.kubeform.com,resources=attacheddatabaseconfigurations/status,verbs=get;update;patch

func (r *AttachedDatabaseConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("attacheddatabaseconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AttachedDatabaseConfiguration")
...
func (r *AttachedDatabaseConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kustov1alpha1.AttachedDatabaseConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", kustov1alpha1.AttachedDatabaseConfiguration{}.APIVersion, kustov1alpha1.AttachedDatabaseConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kustov1alpha1.AttachedDatabaseConfiguration{}).
