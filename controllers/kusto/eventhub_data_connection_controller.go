...
// EventhubDataConnectionReconciler reconciles a EventhubDataConnection object
type EventhubDataConnectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kusto.azurerm.kubeform.com,resources=eventhubdataconnections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kusto.azurerm.kubeform.com,resources=eventhubdataconnections/status,verbs=get;update;patch

func (r *EventhubDataConnectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("eventhubdataconnection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EventhubDataConnection")
...
func (r *EventhubDataConnectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kustov1alpha1.EventhubDataConnection{}); err != nil {
			klog.Error(err, "unable to set up auditor", kustov1alpha1.EventhubDataConnection{}.APIVersion, kustov1alpha1.EventhubDataConnection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kustov1alpha1.EventhubDataConnection{}).
