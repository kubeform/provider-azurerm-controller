...
// InsightsSmartDetectionRuleReconciler reconciles a InsightsSmartDetectionRule object
type InsightsSmartDetectionRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=application.azurerm.kubeform.com,resources=insightssmartdetectionrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=application.azurerm.kubeform.com,resources=insightssmartdetectionrules/status,verbs=get;update;patch

func (r *InsightsSmartDetectionRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("insightssmartdetectionrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InsightsSmartDetectionRule")
...
func (r *InsightsSmartDetectionRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &applicationv1alpha1.InsightsSmartDetectionRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", applicationv1alpha1.InsightsSmartDetectionRule{}.APIVersion, applicationv1alpha1.InsightsSmartDetectionRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&applicationv1alpha1.InsightsSmartDetectionRule{}).
