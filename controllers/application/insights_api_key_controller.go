...
// InsightsAPIKeyReconciler reconciles a InsightsAPIKey object
type InsightsAPIKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=application.azurerm.kubeform.com,resources=insightsapikeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=application.azurerm.kubeform.com,resources=insightsapikeys/status,verbs=get;update;patch

func (r *InsightsAPIKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("insightsapikey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InsightsAPIKey")
...
func (r *InsightsAPIKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &applicationv1alpha1.InsightsAPIKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", applicationv1alpha1.InsightsAPIKey{}.APIVersion, applicationv1alpha1.InsightsAPIKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&applicationv1alpha1.InsightsAPIKey{}).
...
func (r *InsightsAPIKeyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for InsightsAPIKey")
			return nil
		}
		...
		resourceList := &applicationv1alpha1.InsightsAPIKeyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list InsightsAPIKey")
			return nil
		}
