...
// InsightsReconciler reconciles a Insights object
...
// +kubebuilder:rbac:groups=application.azurerm.kubeform.com,resources=insights,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=application.azurerm.kubeform.com,resources=insights/status,verbs=get;update;patch

func (r *InsightsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("insights", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Insights")
...
func (r *InsightsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &applicationv1alpha1.Insights{}); err != nil {
			klog.Error(err, "unable to set up auditor", applicationv1alpha1.Insights{}.APIVersion, applicationv1alpha1.Insights{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&applicationv1alpha1.Insights{}).
...
func (r *InsightsReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Insights")
			return nil
		}
		...
		resourceList := &applicationv1alpha1.InsightsList{}
