...
// InsightsAnalyticsItemReconciler reconciles a InsightsAnalyticsItem object
type InsightsAnalyticsItemReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=application.azurerm.kubeform.com,resources=insightsanalyticsitems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=application.azurerm.kubeform.com,resources=insightsanalyticsitems/status,verbs=get;update;patch

func (r *InsightsAnalyticsItemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("insightsanalyticsitem", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InsightsAnalyticsItem")
...
func (r *InsightsAnalyticsItemReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &applicationv1alpha1.InsightsAnalyticsItem{}); err != nil {
			klog.Error(err, "unable to set up auditor", applicationv1alpha1.InsightsAnalyticsItem{}.APIVersion, applicationv1alpha1.InsightsAnalyticsItem{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&applicationv1alpha1.InsightsAnalyticsItem{}).
