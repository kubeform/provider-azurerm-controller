...
// MlServicesClusterReconciler reconciles a MlServicesCluster object
type MlServicesClusterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=hdinsight.azurerm.kubeform.com,resources=mlservicesclusters/status,verbs=get;update;patch
...
func (r *MlServicesClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MlServicesCluster")
...
func (r *MlServicesClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
...
func (r *MlServicesClusterReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for MlServicesCluster")
			return nil
		}
		...
		resourceList := &hdinsightv1alpha1.MlServicesClusterList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list MlServicesCluster")
			return nil
		}
