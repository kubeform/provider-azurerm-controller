...
// +kubebuilder:rbac:groups=hdinsight.azurerm.kubeform.com,resources=kafkaclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hdinsight.azurerm.kubeform.com,resources=kafkaclusters/status,verbs=get;update;patch
...
func (r *KafkaClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch KafkaCluster")
...
func (r *KafkaClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hdinsightv1alpha1.KafkaCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", hdinsightv1alpha1.KafkaCluster{}.APIVersion, hdinsightv1alpha1.KafkaCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hdinsightv1alpha1.KafkaCluster{}).
...
func (r *KafkaClusterReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for KafkaCluster")
			return nil
		}
		...
		resourceList := &hdinsightv1alpha1.KafkaClusterList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list KafkaCluster")
			return nil
		}
