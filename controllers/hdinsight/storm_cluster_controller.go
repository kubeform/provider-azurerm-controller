...
// +kubebuilder:rbac:groups=hdinsight.azurerm.kubeform.com,resources=stormclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hdinsight.azurerm.kubeform.com,resources=stormclusters/status,verbs=get;update;patch

func (r *StormClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("stormcluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StormCluster")
...
func (r *StormClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hdinsightv1alpha1.StormCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", hdinsightv1alpha1.StormCluster{}.APIVersion, hdinsightv1alpha1.StormCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hdinsightv1alpha1.StormCluster{}).
...
func (r *StormClusterReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for StormCluster")
			return nil
		}
		...
		resourceList := &hdinsightv1alpha1.StormClusterList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list StormCluster")
			return nil
		}
