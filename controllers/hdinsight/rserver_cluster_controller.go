...
// RserverClusterReconciler reconciles a RserverCluster object
type RserverClusterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=hdinsight.azurerm.kubeform.com,resources=rserverclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hdinsight.azurerm.kubeform.com,resources=rserverclusters/status,verbs=get;update;patch

func (r *RserverClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("rservercluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RserverCluster")
...
func (r *RserverClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hdinsightv1alpha1.RserverCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", hdinsightv1alpha1.RserverCluster{}.APIVersion, hdinsightv1alpha1.RserverCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hdinsightv1alpha1.RserverCluster{}).
...
func (r *RserverClusterReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for RserverCluster")
			return nil
		}
		...
		resourceList := &hdinsightv1alpha1.RserverClusterList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list RserverCluster")
			return nil
		}
