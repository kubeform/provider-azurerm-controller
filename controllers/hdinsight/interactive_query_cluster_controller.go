...
// InteractiveQueryClusterReconciler reconciles a InteractiveQueryCluster object
type InteractiveQueryClusterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=hdinsight.azurerm.kubeform.com,resources=interactivequeryclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hdinsight.azurerm.kubeform.com,resources=interactivequeryclusters/status,verbs=get;update;patch

func (r *InteractiveQueryClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("interactivequerycluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InteractiveQueryCluster")
...
func (r *InteractiveQueryClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hdinsightv1alpha1.InteractiveQueryCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", hdinsightv1alpha1.InteractiveQueryCluster{}.APIVersion, hdinsightv1alpha1.InteractiveQueryCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hdinsightv1alpha1.InteractiveQueryCluster{}).
...
func (r *InteractiveQueryClusterReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for InteractiveQueryCluster")
			return nil
		}
		...
		resourceList := &hdinsightv1alpha1.InteractiveQueryClusterList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list InteractiveQueryCluster")
			return nil
		}
