...
// +kubebuilder:rbac:groups=hdinsight.azurerm.kubeform.com,resources=hadoopclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hdinsight.azurerm.kubeform.com,resources=hadoopclusters/status,verbs=get;update;patch

func (r *HadoopClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hadoopcluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HadoopCluster")
...
func (r *HadoopClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hdinsightv1alpha1.HadoopCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", hdinsightv1alpha1.HadoopCluster{}.APIVersion, hdinsightv1alpha1.HadoopCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hdinsightv1alpha1.HadoopCluster{}).
...
func (r *HadoopClusterReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for HadoopCluster")
			return nil
		}
		...
		resourceList := &hdinsightv1alpha1.HadoopClusterList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list HadoopCluster")
			return nil
		}
