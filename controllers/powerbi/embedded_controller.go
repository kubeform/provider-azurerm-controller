...
// EmbeddedReconciler reconciles a Embedded object
...
// +kubebuilder:rbac:groups=powerbi.azurerm.kubeform.com,resources=embeddeds,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=powerbi.azurerm.kubeform.com,resources=embeddeds/status,verbs=get;update;patch

func (r *EmbeddedReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("embedded", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Embedded")
...
func (r *EmbeddedReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &powerbiv1alpha1.Embedded{}); err != nil {
			klog.Error(err, "unable to set up auditor", powerbiv1alpha1.Embedded{}.APIVersion, powerbiv1alpha1.Embedded{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&powerbiv1alpha1.Embedded{}).
