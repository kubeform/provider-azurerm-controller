...
// HealthbotReconciler reconciles a Healthbot object
type HealthbotReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=healthbot.azurerm.kubeform.com,resources=healthbots,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=healthbot.azurerm.kubeform.com,resources=healthbots/status,verbs=get;update;patch

func (r *HealthbotReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("healthbot", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Healthbot")
...
func (r *HealthbotReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &healthbotv1alpha1.Healthbot{}); err != nil {
			klog.Error(err, "unable to set up auditor", healthbotv1alpha1.Healthbot{}.APIVersion, healthbotv1alpha1.Healthbot{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&healthbotv1alpha1.Healthbot{}).
