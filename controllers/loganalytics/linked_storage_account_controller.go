...
// LinkedStorageAccountReconciler reconciles a LinkedStorageAccount object
type LinkedStorageAccountReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=loganalytics.azurerm.kubeform.com,resources=linkedstorageaccounts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=loganalytics.azurerm.kubeform.com,resources=linkedstorageaccounts/status,verbs=get;update;patch

func (r *LinkedStorageAccountReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("linkedstorageaccount", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LinkedStorageAccount")
...
func (r *LinkedStorageAccountReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loganalyticsv1alpha1.LinkedStorageAccount{}); err != nil {
			klog.Error(err, "unable to set up auditor", loganalyticsv1alpha1.LinkedStorageAccount{}.APIVersion, loganalyticsv1alpha1.LinkedStorageAccount{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loganalyticsv1alpha1.LinkedStorageAccount{}).
