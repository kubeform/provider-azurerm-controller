...
// DatasourceWindowsEventReconciler reconciles a DatasourceWindowsEvent object
type DatasourceWindowsEventReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=loganalytics.azurerm.kubeform.com,resources=datasourcewindowsevents,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=loganalytics.azurerm.kubeform.com,resources=datasourcewindowsevents/status,verbs=get;update;patch

func (r *DatasourceWindowsEventReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("datasourcewindowsevent", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DatasourceWindowsEvent")
...
func (r *DatasourceWindowsEventReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loganalyticsv1alpha1.DatasourceWindowsEvent{}); err != nil {
			klog.Error(err, "unable to set up auditor", loganalyticsv1alpha1.DatasourceWindowsEvent{}.APIVersion, loganalyticsv1alpha1.DatasourceWindowsEvent{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loganalyticsv1alpha1.DatasourceWindowsEvent{}).
