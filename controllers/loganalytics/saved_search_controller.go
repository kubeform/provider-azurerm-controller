...
// +kubebuilder:rbac:groups=loganalytics.azurerm.kubeform.com,resources=savedsearches,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=loganalytics.azurerm.kubeform.com,resources=savedsearches/status,verbs=get;update;patch

func (r *SavedSearchReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("savedsearch", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SavedSearch")
...
func (r *SavedSearchReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loganalyticsv1alpha1.SavedSearch{}); err != nil {
			klog.Error(err, "unable to set up auditor", loganalyticsv1alpha1.SavedSearch{}.APIVersion, loganalyticsv1alpha1.SavedSearch{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loganalyticsv1alpha1.SavedSearch{}).
