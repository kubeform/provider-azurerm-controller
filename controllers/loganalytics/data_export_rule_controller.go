...
// DataExportRuleReconciler reconciles a DataExportRule object
type DataExportRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=loganalytics.azurerm.kubeform.com,resources=dataexportrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=loganalytics.azurerm.kubeform.com,resources=dataexportrules/status,verbs=get;update;patch

func (r *DataExportRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dataexportrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DataExportRule")
...
func (r *DataExportRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loganalyticsv1alpha1.DataExportRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", loganalyticsv1alpha1.DataExportRule{}.APIVersion, loganalyticsv1alpha1.DataExportRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loganalyticsv1alpha1.DataExportRule{}).
