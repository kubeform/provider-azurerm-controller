...
// StorageInsightsReconciler reconciles a StorageInsights object
type StorageInsightsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=loganalytics.azurerm.kubeform.com,resources=storageinsights,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=loganalytics.azurerm.kubeform.com,resources=storageinsights/status,verbs=get;update;patch

func (r *StorageInsightsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("storageinsights", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StorageInsights")
...
func (r *StorageInsightsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loganalyticsv1alpha1.StorageInsights{}); err != nil {
			klog.Error(err, "unable to set up auditor", loganalyticsv1alpha1.StorageInsights{}.APIVersion, loganalyticsv1alpha1.StorageInsights{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loganalyticsv1alpha1.StorageInsights{}).
...
func (r *StorageInsightsReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for StorageInsights")
			return nil
		}
		...
		resourceList := &loganalyticsv1alpha1.StorageInsightsList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list StorageInsights")
			return nil
		}
