...
// AssignmentVirtualMachineReconciler reconciles a AssignmentVirtualMachine object
type AssignmentVirtualMachineReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=maintenance.azurerm.kubeform.com,resources=assignmentvirtualmachines,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=maintenance.azurerm.kubeform.com,resources=assignmentvirtualmachines/status,verbs=get;update;patch

func (r *AssignmentVirtualMachineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("assignmentvirtualmachine", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AssignmentVirtualMachine")
...
func (r *AssignmentVirtualMachineReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &maintenancev1alpha1.AssignmentVirtualMachine{}); err != nil {
			klog.Error(err, "unable to set up auditor", maintenancev1alpha1.AssignmentVirtualMachine{}.APIVersion, maintenancev1alpha1.AssignmentVirtualMachine{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&maintenancev1alpha1.AssignmentVirtualMachine{}).
