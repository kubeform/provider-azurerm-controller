...
// AssignmentDedicatedHostReconciler reconciles a AssignmentDedicatedHost object
type AssignmentDedicatedHostReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=maintenance.azurerm.kubeform.com,resources=assignmentdedicatedhosts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=maintenance.azurerm.kubeform.com,resources=assignmentdedicatedhosts/status,verbs=get;update;patch

func (r *AssignmentDedicatedHostReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("assignmentdedicatedhost", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AssignmentDedicatedHost")
...
func (r *AssignmentDedicatedHostReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &maintenancev1alpha1.AssignmentDedicatedHost{}); err != nil {
			klog.Error(err, "unable to set up auditor", maintenancev1alpha1.AssignmentDedicatedHost{}.APIVersion, maintenancev1alpha1.AssignmentDedicatedHost{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&maintenancev1alpha1.AssignmentDedicatedHost{}).
