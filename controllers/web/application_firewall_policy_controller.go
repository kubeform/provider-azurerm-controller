...
// ApplicationFirewallPolicyReconciler reconciles a ApplicationFirewallPolicy object
type ApplicationFirewallPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=web.azurerm.kubeform.com,resources=applicationfirewallpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=web.azurerm.kubeform.com,resources=applicationfirewallpolicies/status,verbs=get;update;patch

func (r *ApplicationFirewallPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("applicationfirewallpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApplicationFirewallPolicy")
...
func (r *ApplicationFirewallPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &webv1alpha1.ApplicationFirewallPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", webv1alpha1.ApplicationFirewallPolicy{}.APIVersion, webv1alpha1.ApplicationFirewallPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&webv1alpha1.ApplicationFirewallPolicy{}).
