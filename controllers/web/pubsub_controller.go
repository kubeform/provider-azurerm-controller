...
// +kubebuilder:rbac:groups=web.azurerm.kubeform.com,resources=pubsubs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=web.azurerm.kubeform.com,resources=pubsubs/status,verbs=get;update;patch

func (r *PubsubReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pubsub", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Pubsub")
...
func (r *PubsubReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &webv1alpha1.Pubsub{}); err != nil {
			klog.Error(err, "unable to set up auditor", webv1alpha1.Pubsub{}.APIVersion, webv1alpha1.Pubsub{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&webv1alpha1.Pubsub{}).
...
func (r *PubsubReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for Pubsub")
			return nil
		}
		...
		resourceList := &webv1alpha1.PubsubList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Pubsub")
			return nil
		}
