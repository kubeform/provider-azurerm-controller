...
// AppSlotReconciler reconciles a AppSlot object
type AppSlotReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=function.azurerm.kubeform.com,resources=appslots,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=function.azurerm.kubeform.com,resources=appslots/status,verbs=get;update;patch

func (r *AppSlotReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("appslot", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AppSlot")
...
func (r *AppSlotReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &functionv1alpha1.AppSlot{}); err != nil {
			klog.Error(err, "unable to set up auditor", functionv1alpha1.AppSlot{}.APIVersion, functionv1alpha1.AppSlot{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&functionv1alpha1.AppSlot{}).
...
func (r *AppSlotReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for AppSlot")
			return nil
		}
		...
		resourceList := &functionv1alpha1.AppSlotList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AppSlot")
			return nil
		}
