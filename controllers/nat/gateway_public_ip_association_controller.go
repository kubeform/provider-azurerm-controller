...
// GatewayPublicIPAssociationReconciler reconciles a GatewayPublicIPAssociation object
type GatewayPublicIPAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=nat.azurerm.kubeform.com,resources=gatewaypublicipassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=nat.azurerm.kubeform.com,resources=gatewaypublicipassociations/status,verbs=get;update;patch

func (r *GatewayPublicIPAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gatewaypublicipassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GatewayPublicIPAssociation")
...
func (r *GatewayPublicIPAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &natv1alpha1.GatewayPublicIPAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", natv1alpha1.GatewayPublicIPAssociation{}.APIVersion, natv1alpha1.GatewayPublicIPAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&natv1alpha1.GatewayPublicIPAssociation{}).
