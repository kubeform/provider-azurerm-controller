...
// +kubebuilder:rbac:groups=container.azurerm.kubeform.com,resources=registrytokens,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=container.azurerm.kubeform.com,resources=registrytokens/status,verbs=get;update;patch

func (r *RegistryTokenReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("registrytoken", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RegistryToken")
...
func (r *RegistryTokenReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &containerv1alpha1.RegistryToken{}); err != nil {
			klog.Error(err, "unable to set up auditor", containerv1alpha1.RegistryToken{}.APIVersion, containerv1alpha1.RegistryToken{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&containerv1alpha1.RegistryToken{}).
