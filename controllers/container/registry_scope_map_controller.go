...
// RegistryScopeMapReconciler reconciles a RegistryScopeMap object
type RegistryScopeMapReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=container.azurerm.kubeform.com,resources=registryscopemaps,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=container.azurerm.kubeform.com,resources=registryscopemaps/status,verbs=get;update;patch

func (r *RegistryScopeMapReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("registryscopemap", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RegistryScopeMap")
...
func (r *RegistryScopeMapReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &containerv1alpha1.RegistryScopeMap{}); err != nil {
			klog.Error(err, "unable to set up auditor", containerv1alpha1.RegistryScopeMap{}.APIVersion, containerv1alpha1.RegistryScopeMap{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&containerv1alpha1.RegistryScopeMap{}).
