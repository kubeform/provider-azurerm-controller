...
// RegistryTaskReconciler reconciles a RegistryTask object
type RegistryTaskReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=container.azurerm.kubeform.com,resources=registrytasks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=container.azurerm.kubeform.com,resources=registrytasks/status,verbs=get;update;patch

func (r *RegistryTaskReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("registrytask", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RegistryTask")
...
func (r *RegistryTaskReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &containerv1alpha1.RegistryTask{}); err != nil {
			klog.Error(err, "unable to set up auditor", containerv1alpha1.RegistryTask{}.APIVersion, containerv1alpha1.RegistryTask{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&containerv1alpha1.RegistryTask{}).
...
func (r *RegistryTaskReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for RegistryTask")
			return nil
		}
		...
		resourceList := &containerv1alpha1.RegistryTaskList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list RegistryTask")
			return nil
		}
