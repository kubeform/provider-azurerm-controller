...
// TenantConfigurationReconciler reconciles a TenantConfiguration object
type TenantConfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=portal.azurerm.kubeform.com,resources=tenantconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=portal.azurerm.kubeform.com,resources=tenantconfigurations/status,verbs=get;update;patch

func (r *TenantConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tenantconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TenantConfiguration")
...
func (r *TenantConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &portalv1alpha1.TenantConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", portalv1alpha1.TenantConfiguration{}.APIVersion, portalv1alpha1.TenantConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&portalv1alpha1.TenantConfiguration{}).
