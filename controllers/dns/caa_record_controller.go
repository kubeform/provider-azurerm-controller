...
// CaaRecordReconciler reconciles a CaaRecord object
type CaaRecordReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dns.azurerm.kubeform.com,resources=caarecords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.azurerm.kubeform.com,resources=caarecords/status,verbs=get;update;patch

func (r *CaaRecordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("caarecord", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CaaRecord")
...
func (r *CaaRecordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.CaaRecord{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.CaaRecord{}.APIVersion, dnsv1alpha1.CaaRecord{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dnsv1alpha1.CaaRecord{}).
