...
// NsRecordReconciler reconciles a NsRecord object
type NsRecordReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dns.azurerm.kubeform.com,resources=nsrecords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.azurerm.kubeform.com,resources=nsrecords/status,verbs=get;update;patch

func (r *NsRecordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("nsrecord", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NsRecord")
...
func (r *NsRecordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.NsRecord{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.NsRecord{}.APIVersion, dnsv1alpha1.NsRecord{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dnsv1alpha1.NsRecord{}).
