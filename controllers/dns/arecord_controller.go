...
// +kubebuilder:rbac:groups=dns.azurerm.kubeform.com,resources=arecords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.azurerm.kubeform.com,resources=arecords/status,verbs=get;update;patch

func (r *ARecordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("arecord", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ARecord")
...
func (r *ARecordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.ARecord{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.ARecord{}.APIVersion, dnsv1alpha1.ARecord{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dnsv1alpha1.ARecord{}).
