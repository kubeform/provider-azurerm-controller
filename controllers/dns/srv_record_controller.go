...
// SrvRecordReconciler reconciles a SrvRecord object
type SrvRecordReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dns.azurerm.kubeform.com,resources=srvrecords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.azurerm.kubeform.com,resources=srvrecords/status,verbs=get;update;patch

func (r *SrvRecordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("srvrecord", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SrvRecord")
...
func (r *SrvRecordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.SrvRecord{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.SrvRecord{}.APIVersion, dnsv1alpha1.SrvRecord{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dnsv1alpha1.SrvRecord{}).
