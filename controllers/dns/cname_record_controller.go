...
// CnameRecordReconciler reconciles a CnameRecord object
...
// +kubebuilder:rbac:groups=dns.azurerm.kubeform.com,resources=cnamerecords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.azurerm.kubeform.com,resources=cnamerecords/status,verbs=get;update;patch

func (r *CnameRecordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cnamerecord", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CnameRecord")
...
func (r *CnameRecordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.CnameRecord{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.CnameRecord{}.APIVersion, dnsv1alpha1.CnameRecord{}.Kind)
			return err
		}
	}
