...
// +kubebuilder:rbac:groups=media.azurerm.kubeform.com,resources=transforms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=media.azurerm.kubeform.com,resources=transforms/status,verbs=get;update;patch

func (r *TransformReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("transform", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Transform")
...
func (r *TransformReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mediav1alpha1.Transform{}); err != nil {
			klog.Error(err, "unable to set up auditor", mediav1alpha1.Transform{}.APIVersion, mediav1alpha1.Transform{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mediav1alpha1.Transform{}).
