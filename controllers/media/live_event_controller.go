...
// LiveEventReconciler reconciles a LiveEvent object
type LiveEventReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=media.azurerm.kubeform.com,resources=liveevents,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=media.azurerm.kubeform.com,resources=liveevents/status,verbs=get;update;patch

func (r *LiveEventReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("liveevent", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LiveEvent")
...
func (r *LiveEventReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mediav1alpha1.LiveEvent{}); err != nil {
			klog.Error(err, "unable to set up auditor", mediav1alpha1.LiveEvent{}.APIVersion, mediav1alpha1.LiveEvent{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mediav1alpha1.LiveEvent{}).
