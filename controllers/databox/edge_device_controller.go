...
// +kubebuilder:rbac:groups=databox.azurerm.kubeform.com,resources=edgedevices,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=databox.azurerm.kubeform.com,resources=edgedevices/status,verbs=get;update;patch

func (r *EdgeDeviceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("edgedevice", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EdgeDevice")
...
func (r *EdgeDeviceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databoxv1alpha1.EdgeDevice{}); err != nil {
			klog.Error(err, "unable to set up auditor", databoxv1alpha1.EdgeDevice{}.APIVersion, databoxv1alpha1.EdgeDevice{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databoxv1alpha1.EdgeDevice{}).
