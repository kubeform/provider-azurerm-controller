...
// EdgeOrderReconciler reconciles a EdgeOrder object
type EdgeOrderReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=databox.azurerm.kubeform.com,resources=edgeorders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=databox.azurerm.kubeform.com,resources=edgeorders/status,verbs=get;update;patch

func (r *EdgeOrderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("edgeorder", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EdgeOrder")
...
func (r *EdgeOrderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databoxv1alpha1.EdgeOrder{}); err != nil {
			klog.Error(err, "unable to set up auditor", databoxv1alpha1.EdgeOrder{}.APIVersion, databoxv1alpha1.EdgeOrder{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databoxv1alpha1.EdgeOrder{}).
