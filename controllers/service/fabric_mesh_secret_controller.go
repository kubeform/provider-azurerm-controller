...
// FabricMeshSecretReconciler reconciles a FabricMeshSecret object
type FabricMeshSecretReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=service.azurerm.kubeform.com,resources=fabricmeshsecrets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=service.azurerm.kubeform.com,resources=fabricmeshsecrets/status,verbs=get;update;patch

func (r *FabricMeshSecretReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fabricmeshsecret", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FabricMeshSecret")
...
func (r *FabricMeshSecretReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicev1alpha1.FabricMeshSecret{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicev1alpha1.FabricMeshSecret{}.APIVersion, servicev1alpha1.FabricMeshSecret{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicev1alpha1.FabricMeshSecret{}).
