...
// FabricClusterReconciler reconciles a FabricCluster object
type FabricClusterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=service.azurerm.kubeform.com,resources=fabricclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=service.azurerm.kubeform.com,resources=fabricclusters/status,verbs=get;update;patch

func (r *FabricClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fabriccluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FabricCluster")
...
func (r *FabricClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicev1alpha1.FabricCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicev1alpha1.FabricCluster{}.APIVersion, servicev1alpha1.FabricCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicev1alpha1.FabricCluster{}).
