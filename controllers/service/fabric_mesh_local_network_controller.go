...
// FabricMeshLocalNetworkReconciler reconciles a FabricMeshLocalNetwork object
type FabricMeshLocalNetworkReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=service.azurerm.kubeform.com,resources=fabricmeshlocalnetworks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=service.azurerm.kubeform.com,resources=fabricmeshlocalnetworks/status,verbs=get;update;patch

func (r *FabricMeshLocalNetworkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fabricmeshlocalnetwork", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FabricMeshLocalNetwork")
...
func (r *FabricMeshLocalNetworkReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicev1alpha1.FabricMeshLocalNetwork{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicev1alpha1.FabricMeshLocalNetwork{}.APIVersion, servicev1alpha1.FabricMeshLocalNetwork{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicev1alpha1.FabricMeshLocalNetwork{}).
