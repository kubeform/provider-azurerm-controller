...
// ProviderRegistrationReconciler reconciles a ProviderRegistration object
type ProviderRegistrationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=resource.azurerm.kubeform.com,resources=providerregistrations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=resource.azurerm.kubeform.com,resources=providerregistrations/status,verbs=get;update;patch

func (r *ProviderRegistrationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("providerregistration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProviderRegistration")
...
func (r *ProviderRegistrationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &resourcev1alpha1.ProviderRegistration{}); err != nil {
			klog.Error(err, "unable to set up auditor", resourcev1alpha1.ProviderRegistration{}.APIVersion, resourcev1alpha1.ProviderRegistration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&resourcev1alpha1.ProviderRegistration{}).
