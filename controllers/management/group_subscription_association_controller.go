...
// GroupSubscriptionAssociationReconciler reconciles a GroupSubscriptionAssociation object
type GroupSubscriptionAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=management.azurerm.kubeform.com,resources=groupsubscriptionassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=management.azurerm.kubeform.com,resources=groupsubscriptionassociations/status,verbs=get;update;patch

func (r *GroupSubscriptionAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("groupsubscriptionassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GroupSubscriptionAssociation")
...
func (r *GroupSubscriptionAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &managementv1alpha1.GroupSubscriptionAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", managementv1alpha1.GroupSubscriptionAssociation{}.APIVersion, managementv1alpha1.GroupSubscriptionAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&managementv1alpha1.GroupSubscriptionAssociation{}).
