...
// GroupPolicyAssignmentReconciler reconciles a GroupPolicyAssignment object
type GroupPolicyAssignmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=management.azurerm.kubeform.com,resources=grouppolicyassignments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=management.azurerm.kubeform.com,resources=grouppolicyassignments/status,verbs=get;update;patch

func (r *GroupPolicyAssignmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("grouppolicyassignment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GroupPolicyAssignment")
...
func (r *GroupPolicyAssignmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &managementv1alpha1.GroupPolicyAssignment{}); err != nil {
			klog.Error(err, "unable to set up auditor", managementv1alpha1.GroupPolicyAssignment{}.APIVersion, managementv1alpha1.GroupPolicyAssignment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&managementv1alpha1.GroupPolicyAssignment{}).
