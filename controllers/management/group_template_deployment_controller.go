...
// GroupTemplateDeploymentReconciler reconciles a GroupTemplateDeployment object
type GroupTemplateDeploymentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=management.azurerm.kubeform.com,resources=grouptemplatedeployments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=management.azurerm.kubeform.com,resources=grouptemplatedeployments/status,verbs=get;update;patch

func (r *GroupTemplateDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("grouptemplatedeployment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GroupTemplateDeployment")
...
func (r *GroupTemplateDeploymentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &managementv1alpha1.GroupTemplateDeployment{}); err != nil {
			klog.Error(err, "unable to set up auditor", managementv1alpha1.GroupTemplateDeployment{}.APIVersion, managementv1alpha1.GroupTemplateDeployment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&managementv1alpha1.GroupTemplateDeployment{}).
