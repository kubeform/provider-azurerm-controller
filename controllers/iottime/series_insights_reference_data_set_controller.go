...
// SeriesInsightsReferenceDataSetReconciler reconciles a SeriesInsightsReferenceDataSet object
type SeriesInsightsReferenceDataSetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iottime.azurerm.kubeform.com,resources=seriesinsightsreferencedatasets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iottime.azurerm.kubeform.com,resources=seriesinsightsreferencedatasets/status,verbs=get;update;patch

func (r *SeriesInsightsReferenceDataSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("seriesinsightsreferencedataset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SeriesInsightsReferenceDataSet")
...
func (r *SeriesInsightsReferenceDataSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iottimev1alpha1.SeriesInsightsReferenceDataSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", iottimev1alpha1.SeriesInsightsReferenceDataSet{}.APIVersion, iottimev1alpha1.SeriesInsightsReferenceDataSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iottimev1alpha1.SeriesInsightsReferenceDataSet{}).
