...
// SeriesInsightsAccessPolicyReconciler reconciles a SeriesInsightsAccessPolicy object
type SeriesInsightsAccessPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iottime.azurerm.kubeform.com,resources=seriesinsightsaccesspolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iottime.azurerm.kubeform.com,resources=seriesinsightsaccesspolicies/status,verbs=get;update;patch

func (r *SeriesInsightsAccessPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("seriesinsightsaccesspolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SeriesInsightsAccessPolicy")
...
func (r *SeriesInsightsAccessPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iottimev1alpha1.SeriesInsightsAccessPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", iottimev1alpha1.SeriesInsightsAccessPolicy{}.APIVersion, iottimev1alpha1.SeriesInsightsAccessPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iottimev1alpha1.SeriesInsightsAccessPolicy{}).
