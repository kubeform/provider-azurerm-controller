...
// SqlFunctionReconciler reconciles a SqlFunction object
type SqlFunctionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cosmosdb.azurerm.kubeform.com,resources=sqlfunctions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cosmosdb.azurerm.kubeform.com,resources=sqlfunctions/status,verbs=get;update;patch

func (r *SqlFunctionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sqlfunction", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SqlFunction")
...
func (r *SqlFunctionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cosmosdbv1alpha1.SqlFunction{}); err != nil {
			klog.Error(err, "unable to set up auditor", cosmosdbv1alpha1.SqlFunction{}.APIVersion, cosmosdbv1alpha1.SqlFunction{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cosmosdbv1alpha1.SqlFunction{}).
