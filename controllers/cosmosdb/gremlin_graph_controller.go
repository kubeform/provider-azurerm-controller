...
// GremlinGraphReconciler reconciles a GremlinGraph object
type GremlinGraphReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cosmosdb.azurerm.kubeform.com,resources=gremlingraphs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cosmosdb.azurerm.kubeform.com,resources=gremlingraphs/status,verbs=get;update;patch

func (r *GremlinGraphReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gremlingraph", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GremlinGraph")
...
func (r *GremlinGraphReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cosmosdbv1alpha1.GremlinGraph{}); err != nil {
			klog.Error(err, "unable to set up auditor", cosmosdbv1alpha1.GremlinGraph{}.APIVersion, cosmosdbv1alpha1.GremlinGraph{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cosmosdbv1alpha1.GremlinGraph{}).
