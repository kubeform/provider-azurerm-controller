...
// +kubebuilder:rbac:groups=cosmosdb.azurerm.kubeform.com,resources=mongodatabases,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cosmosdb.azurerm.kubeform.com,resources=mongodatabases/status,verbs=get;update;patch

func (r *MongoDatabaseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("mongodatabase", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MongoDatabase")
...
func (r *MongoDatabaseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cosmosdbv1alpha1.MongoDatabase{}); err != nil {
			klog.Error(err, "unable to set up auditor", cosmosdbv1alpha1.MongoDatabase{}.APIVersion, cosmosdbv1alpha1.MongoDatabase{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cosmosdbv1alpha1.MongoDatabase{}).
