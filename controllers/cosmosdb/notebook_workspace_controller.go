...
// NotebookWorkspaceReconciler reconciles a NotebookWorkspace object
type NotebookWorkspaceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cosmosdb.azurerm.kubeform.com,resources=notebookworkspaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cosmosdb.azurerm.kubeform.com,resources=notebookworkspaces/status,verbs=get;update;patch

func (r *NotebookWorkspaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("notebookworkspace", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NotebookWorkspace")
...
func (r *NotebookWorkspaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cosmosdbv1alpha1.NotebookWorkspace{}); err != nil {
			klog.Error(err, "unable to set up auditor", cosmosdbv1alpha1.NotebookWorkspace{}.APIVersion, cosmosdbv1alpha1.NotebookWorkspace{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cosmosdbv1alpha1.NotebookWorkspace{}).
