...
// SqlStoredProcedureReconciler reconciles a SqlStoredProcedure object
type SqlStoredProcedureReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cosmosdb.azurerm.kubeform.com,resources=sqlstoredprocedures,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cosmosdb.azurerm.kubeform.com,resources=sqlstoredprocedures/status,verbs=get;update;patch

func (r *SqlStoredProcedureReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sqlstoredprocedure", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SqlStoredProcedure")
...
func (r *SqlStoredProcedureReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cosmosdbv1alpha1.SqlStoredProcedure{}); err != nil {
			klog.Error(err, "unable to set up auditor", cosmosdbv1alpha1.SqlStoredProcedure{}.APIVersion, cosmosdbv1alpha1.SqlStoredProcedure{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cosmosdbv1alpha1.SqlStoredProcedure{}).
