...
// CassandraKeyspaceReconciler reconciles a CassandraKeyspace object
type CassandraKeyspaceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cosmosdb.azurerm.kubeform.com,resources=cassandrakeyspaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cosmosdb.azurerm.kubeform.com,resources=cassandrakeyspaces/status,verbs=get;update;patch

func (r *CassandraKeyspaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cassandrakeyspace", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CassandraKeyspace")
...
func (r *CassandraKeyspaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cosmosdbv1alpha1.CassandraKeyspace{}); err != nil {
			klog.Error(err, "unable to set up auditor", cosmosdbv1alpha1.CassandraKeyspace{}.APIVersion, cosmosdbv1alpha1.CassandraKeyspace{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cosmosdbv1alpha1.CassandraKeyspace{}).
