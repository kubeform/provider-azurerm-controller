...
// CreatorReconciler reconciles a Creator object
type CreatorReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=maps.azurerm.kubeform.com,resources=creators,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=maps.azurerm.kubeform.com,resources=creators/status,verbs=get;update;patch

func (r *CreatorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("creator", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Creator")
...
func (r *CreatorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mapsv1alpha1.Creator{}); err != nil {
			klog.Error(err, "unable to set up auditor", mapsv1alpha1.Creator{}.APIVersion, mapsv1alpha1.Creator{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mapsv1alpha1.Creator{}).
