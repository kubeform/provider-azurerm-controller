...
// ThreatProtectionReconciler reconciles a ThreatProtection object
type ThreatProtectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=advanced.azurerm.kubeform.com,resources=threatprotections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=advanced.azurerm.kubeform.com,resources=threatprotections/status,verbs=get;update;patch

func (r *ThreatProtectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("threatprotection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ThreatProtection")
...
func (r *ThreatProtectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &advancedv1alpha1.ThreatProtection{}); err != nil {
			klog.Error(err, "unable to set up auditor", advancedv1alpha1.ThreatProtection{}.APIVersion, advancedv1alpha1.ThreatProtection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&advancedv1alpha1.ThreatProtection{}).
