...
// EndpointCustomDomainReconciler reconciles a EndpointCustomDomain object
type EndpointCustomDomainReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cdn.azurerm.kubeform.com,resources=endpointcustomdomains,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cdn.azurerm.kubeform.com,resources=endpointcustomdomains/status,verbs=get;update;patch

func (r *EndpointCustomDomainReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("endpointcustomdomain", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EndpointCustomDomain")
...
func (r *EndpointCustomDomainReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cdnv1alpha1.EndpointCustomDomain{}); err != nil {
			klog.Error(err, "unable to set up auditor", cdnv1alpha1.EndpointCustomDomain{}.APIVersion, cdnv1alpha1.EndpointCustomDomain{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cdnv1alpha1.EndpointCustomDomain{}).
