...
// BackendAddressPoolReconciler reconciles a BackendAddressPool object
type BackendAddressPoolReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=lb.azurerm.kubeform.com,resources=backendaddresspools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lb.azurerm.kubeform.com,resources=backendaddresspools/status,verbs=get;update;patch

func (r *BackendAddressPoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("backendaddresspool", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BackendAddressPool")
...
func (r *BackendAddressPoolReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lbv1alpha1.BackendAddressPool{}); err != nil {
			klog.Error(err, "unable to set up auditor", lbv1alpha1.BackendAddressPool{}.APIVersion, lbv1alpha1.BackendAddressPool{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lbv1alpha1.BackendAddressPool{}).
