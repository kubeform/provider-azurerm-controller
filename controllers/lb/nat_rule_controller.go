...
// +kubebuilder:rbac:groups=lb.azurerm.kubeform.com,resources=natrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lb.azurerm.kubeform.com,resources=natrules/status,verbs=get;update;patch

func (r *NatRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("natrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NatRule")
...
func (r *NatRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lbv1alpha1.NatRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", lbv1alpha1.NatRule{}.APIVersion, lbv1alpha1.NatRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lbv1alpha1.NatRule{}).
