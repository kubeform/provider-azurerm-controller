...
// OutboundRuleReconciler reconciles a OutboundRule object
type OutboundRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=lb.azurerm.kubeform.com,resources=outboundrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lb.azurerm.kubeform.com,resources=outboundrules/status,verbs=get;update;patch

func (r *OutboundRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("outboundrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OutboundRule")
...
func (r *OutboundRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lbv1alpha1.OutboundRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", lbv1alpha1.OutboundRule{}.APIVersion, lbv1alpha1.OutboundRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lbv1alpha1.OutboundRule{}).
