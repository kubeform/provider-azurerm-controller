...
// NatPoolReconciler reconciles a NatPool object
type NatPoolReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=lb.azurerm.kubeform.com,resources=natpools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lb.azurerm.kubeform.com,resources=natpools/status,verbs=get;update;patch

func (r *NatPoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("natpool", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NatPool")
...
func (r *NatPoolReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lbv1alpha1.NatPool{}); err != nil {
			klog.Error(err, "unable to set up auditor", lbv1alpha1.NatPool{}.APIVersion, lbv1alpha1.NatPool{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lbv1alpha1.NatPool{}).
