...
func dataSourceLinodeVLANsRead(d *schema.ResourceData, meta interface{}) error {
	...
	vlans, err := client.ListVLANs(context.Background(), &azurermgo.ListOptions{
		Filter: filter,
	})

	if err != nil {
		return fmt.Errorf("failed to list azurerm vlans: %s", err)
	}
...
func flattenLinodeVLAN(vlan *azurermgo.VLAN) map[string]interface{} {
	result := make(map[string]interface{})

	result["label"] = vlan.Label
	result["azurerms"] = vlan.Linodes
