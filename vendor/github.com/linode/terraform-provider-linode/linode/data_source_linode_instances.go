	...
	return &schema.Resource{
		Schema: map[string]*schema.Schema{
			"image": {
				...
				Description: "An Image ID to deploy the Disk from. Official Linode Images start with azurerm/, while " +
...
func dataSourceLinodeInstancesRead(d *schema.ResourceData, meta interface{}) error {
	...
	instances, err := client.ListInstances(context.Background(), &azurermgo.ListOptions{
		Filter: filter,
	})
	if err != nil {
...
func flattenLinodeInstance(client *azurermgo.Client, instance *azurermgo.Instance) (map[string]interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get ips for azurerm instance %d: %s", id, err)
	}
