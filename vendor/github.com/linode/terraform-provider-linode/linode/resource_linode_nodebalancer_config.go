	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"protocol": {
				...
				Optional: true,
				Default:  azurermgo.ProtocolHTTP,
			},
			"proxy_protocol": {
				Type: schema.TypeString,
				...
				Optional:     true,
				Default:      azurermgo.ProxyProtocolNone,
			},
			"port": {
				Type: schema.TypeInt,
...
func resourceLinodeNodeBalancerConfigCreate(d *schema.ResourceData, meta interface{}) error {
	...
	createOpts := azurermgo.NodeBalancerConfigCreateOptions{
		Algorithm:     azurermgo.ConfigAlgorithm(d.Get("algorithm").(string)),
		Check:         azurermgo.ConfigCheck(d.Get("check").(string)),
		Stickiness:    azurermgo.ConfigStickiness(d.Get("stickiness").(string)),
		...
		Port:          d.Get("port").(int),
		Protocol:      azurermgo.ConfigProtocol(strings.ToLower(d.Get("protocol").(string))),
		ProxyProtocol: azurermgo.ConfigProxyProtocol(d.Get("proxy_protocol").(string)),
...
func resourceLinodeNodeBalancerConfigUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	updateOpts := azurermgo.NodeBalancerConfigUpdateOptions{
		Algorithm:     azurermgo.ConfigAlgorithm(d.Get("algorithm").(string)),
		Check:         azurermgo.ConfigCheck(d.Get("check").(string)),
		Stickiness:    azurermgo.ConfigStickiness(d.Get("stickiness").(string)),
		CheckAttempts: d.Get("check_attempts").(int),
