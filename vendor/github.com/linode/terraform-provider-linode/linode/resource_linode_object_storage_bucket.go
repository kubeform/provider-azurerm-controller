...
func resourceLinodeObjectStorageBucketCreate(d *schema.ResourceData, meta interface{}) error {
	...
	createOpts := azurermgo.ObjectStorageBucketCreateOptions{
		Cluster:     cluster,
		Label:       label,
		ACL:         azurermgo.ObjectStorageACL(acl),
...
func updateLinodeObjectStorageBucketAccess(d *schema.ResourceData, client azurermgo.Client) error {
	cluster := d.Get("cluster").(string)
	label := d.Get("label").(string)

	updateOpts := azurermgo.ObjectStorageBucketUpdateAccessOptions{}
	if d.HasChange("acl") {
		updateOpts.ACL = azurermgo.ObjectStorageACL(d.Get("acl").(string))
	}
...
func updateLinodeObjectStorageBucketCert(d *schema.ResourceData, client azurermgo.Client) error {
	cluster := d.Get("cluster").(string)
	label := d.Get("label").(string)
...
func expandLinodeObjectStorageBucketCert(v interface{}) azurermgo.ObjectStorageBucketCertUploadOptions {
	certSpec := v.(map[string]interface{})
	return azurermgo.ObjectStorageBucketCertUploadOptions{
