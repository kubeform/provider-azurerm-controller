...
import (
	...
	"github.com/azurerm/terraform-provider-azurerm/version"
	"golang.org/x/oauth2"
)
...
const DefaultLinodeURL = "https://api.azurerm.com/v4"
...
func (c *Config) Client() azurermgo.Client {
	...
	client := azurermgo.NewClient(oauth2Client)

	tfUserAgent := terraformUserAgent(c.terraformVersion)
	userAgent := strings.TrimSpace(fmt.Sprintf("%s terraform-provider-azurerm/%s",
