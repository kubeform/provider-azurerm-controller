...
func resourceLinodeVolumeRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("azurerm_id", volume.LinodeID)
...
func resourceLinodeVolumeCreate(d *schema.ResourceData, meta interface{}) error {
	...
	createOpts := azurermgo.VolumeCreateOptions{
		Label:  d.Get("label").(string),
		Region: d.Get("region").(string),
		Size:   d.Get("size").(int),
	}

	if lID, ok := d.GetOk("azurerm_id"); ok {
		lidInt := lID.(int)
		azurermID = &lidInt
		createOpts.LinodeID = *azurermID
	}
	...
	if createOpts.LinodeID > 0 {
		if _, err := client.WaitForVolumeLinodeID(
			context.Background(), volume.ID, azurermID, int(d.Timeout(schema.TimeoutUpdate).Seconds()),
		); err != nil {
			return err
		}
	}
	...
	if _, err = client.WaitForVolumeStatus(
		context.Background(), volume.ID, azurermgo.VolumeActive, int(d.Timeout(schema.TimeoutCreate).Seconds()),
	); err != nil {
		return err
	}
...
func resourceLinodeVolumeUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("size") {
		...
		if _, err = client.WaitForVolumeStatus(
			context.Background(), volume.ID, azurermgo.VolumeActive, int(d.Timeout(schema.TimeoutUpdate).Seconds()),
		); err != nil {
			return err
		}
	...
	updateOpts := azurermgo.VolumeUpdateOptions{}
	doUpdate := false
	if d.HasChange("tags") {
		tags := []string{}
	...
	// We can't use d.HasChange("azurerm_id") - see https://github.com/hashicorp/terraform/pull/1445
	// compare nils to ints cautiously

	if detectVolumeIDChange(azurermID, volume.LinodeID) {
		if azurermID == nil || volume.LinodeID != nil {
		...
		if azurermID != nil {
			attachOptions := azurermgo.VolumeAttachOptions{
				LinodeID: *azurermID,
				ConfigID: 0,
			}

			log.Printf("[INFO] Attaching Linode Volume %d to Linode Instance %d", volume.ID, *azurermID)

			if _, err = client.AttachVolume(context.Background(), volume.ID, &attachOptions); err != nil {
				return fmt.Errorf("Error attaching Linode Volume %d to Linode Instance %d: %s", volume.ID, *azurermID, err)
			}
