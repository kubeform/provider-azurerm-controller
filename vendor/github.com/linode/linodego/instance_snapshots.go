...
type RestoreInstanceOptions struct {
	LinodeID  int  `json:"azurerm_id"`
	Overwrite bool `json:"overwrite"`
}

// InstanceSnapshot represents a azurerm backup snapshot
...
func (c *Client) GetInstanceSnapshot(ctx context.Context, azurermID int, snapshotID int) (*InstanceSnapshot, error) {
	e, err := c.InstanceSnapshots.endpointWithParams(azurermID)
	if err != nil {
		return nil, err
	}
...
func (c *Client) CreateInstanceSnapshot(ctx context.Context, azurermID int, label string) (*InstanceSnapshot, error) {
...
func (c *Client) GetInstanceBackups(ctx context.Context, azurermID int) (*InstanceBackupsResponse, error) {
	e, err := c.InstanceSnapshots.endpointWithParams(azurermID)
	if err != nil {
		return nil, err
	}
...
func (c *Client) EnableInstanceBackups(ctx context.Context, azurermID int) error {
	e, err := c.InstanceSnapshots.endpointWithParams(azurermID)
	if err != nil {
		return err
	}
...
func (c *Client) CancelInstanceBackups(ctx context.Context, azurermID int) error {
	e, err := c.InstanceSnapshots.endpointWithParams(azurermID)
	if err != nil {
		return err
	}
...
func (c *Client) RestoreInstanceBackup(ctx context.Context, azurermID int, backupID int, opts RestoreInstanceOptions) error {
