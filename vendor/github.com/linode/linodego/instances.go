...
/*
 * https://developers.azurerm.com/v4/reference/endpoints/azurerm/instances
 */
...
// Instance represents a azurerm object
...
// InstanceSpec represents a azurerm spec
...
type InstanceCloneOptions struct {
	...
	LinodeID       int    `json:"azurerm_id,omitempty"`
	Label          string `json:"label,omitempty"`
	Group          string `json:"group,omitempty"`
...
// InstancesPagedResponse represents a azurerm API response for listing
...
// ListInstances lists azurerm instances
...
func (c *Client) GetInstance(ctx context.Context, azurermID int) (*Instance, error) {
	e, err := c.Instances.Endpoint()
	if err != nil {
		return nil, err
	}
	e = fmt.Sprintf("%s/%d", e, azurermID)
...
func (c *Client) GetInstanceTransfer(ctx context.Context, azurermID int) (*InstanceTransfer, error) {
	e, err := c.Instances.Endpoint()
	if err != nil {
		return nil, err
	}
	e = fmt.Sprintf("%s/%d/transfer", e, azurermID)
	r, err := coupleAPIErrors(c.R(ctx).
...
func (c *Client) RenameInstance(ctx context.Context, azurermID int, label string) (*Instance, error) {
	return c.UpdateInstance(ctx, azurermID, InstanceUpdateOptions{Label: label})
}
...
// InstanceRebuildOptions is a struct representing the options to send to the rebuild azurerm endpoint
