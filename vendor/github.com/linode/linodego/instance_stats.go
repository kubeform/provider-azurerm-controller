...
func (c *Client) GetInstanceStats(ctx context.Context, azurermID int) (*InstanceStats, error) {
	e, err := c.InstanceStats.endpointWithParams(azurermID)
	if err != nil {
		return nil, err
	}
...
func (c *Client) GetInstanceStatsByDate(ctx context.Context, azurermID int, year int, month int) (*InstanceStats, error) {
	e := endpointWithIDAndDate(c, azurermID, year, month)
	r, err := coupleAPIErrors(c.R(ctx).SetResult(&InstanceStats{}).Get(e))
	if err != nil {
		return nil, err
	}
