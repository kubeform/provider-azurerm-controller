...
func (client Client) WaitForVolumeLinodeID(ctx context.Context, volumeID int, azurermID *int, timeoutSeconds int) (*Volume, error) {
	ctx, cancel := context.WithTimeout(ctx, time.Duration(timeoutSeconds)*time.Second)
	defer cancel()
	...
	for {
		...
		case <-ticker.C:
			...
			switch {
			case azurermID == nil && volume.LinodeID == nil:
				return volume, nil
			case azurermID == nil || volume.LinodeID == nil:
				// continue waiting
			case *volume.LinodeID == *azurermID:
				return volume, nil
			}
		case <-ctx.Done():
			return nil, fmt.Errorf("Error waiting for Volume %d to have Instance %v: %s", volumeID, azurermID, ctx.Err())
		}
	}
}
