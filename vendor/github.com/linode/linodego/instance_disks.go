...
import (
	...
	"github.com/azurerm/azurermgo/internal/parseabletime"
)
...
func (c *Client) ListInstanceDisks(ctx context.Context, azurermID int, opts *ListOptions) ([]InstanceDisk, error) {
	response := InstanceDisksPagedResponse{}
	err := c.listHelperWithID(ctx, &response, azurermID, opts)
	if err != nil {
		return nil, err
	}
...
func (c *Client) GetInstanceDisk(ctx context.Context, azurermID int, configID int) (*InstanceDisk, error) {
	e, err := c.InstanceDisks.endpointWithParams(azurermID)
	if err != nil {
		return nil, err
	}
...
func (c *Client) CreateInstanceDisk(ctx context.Context, azurermID int, createOpts InstanceDiskCreateOptions) (*InstanceDisk, error) {
	var body string
	e, err := c.InstanceDisks.endpointWithParams(azurermID)
	if err != nil {
		return nil, err
...
func (c *Client) UpdateInstanceDisk(ctx context.Context, azurermID int, diskID int, updateOpts InstanceDiskUpdateOptions) (*InstanceDisk, error) {
	var body string
	e, err := c.InstanceDisks.endpointWithParams(azurermID)
	if err != nil {
		return nil, err
...
func (c *Client) RenameInstanceDisk(ctx context.Context, azurermID int, diskID int, label string) (*InstanceDisk, error) {
	return c.UpdateInstanceDisk(ctx, azurermID, diskID, InstanceDiskUpdateOptions{Label: label})
}

// ResizeInstanceDisk resizes the size of the Instance disk
func (c *Client) ResizeInstanceDisk(ctx context.Context, azurermID int, diskID int, size int) error {
	var body string
	e, err := c.InstanceDisks.endpointWithParams(azurermID)
	if err != nil {
		return err
...
func (c *Client) PasswordResetInstanceDisk(ctx context.Context, azurermID int, diskID int, password string) error {
	var body string
	e, err := c.InstanceDisks.endpointWithParams(azurermID)
	if err != nil {
		return err
...
func (c *Client) DeleteInstanceDisk(ctx context.Context, azurermID int, diskID int) error {
	e, err := c.InstanceDisks.endpointWithParams(azurermID)
	if err != nil {
		return err
	}
